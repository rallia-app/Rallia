...
/**
 * One day, one sport, one creation-mode (auto vs organic) bucket of the match
 * lifecycle funnel, judged at closure (~48h after match end). Bucketed by play
 * date (match_date). See get_match_quality_analytics migration for the strict
 * "completed quality game" definition and the feedback-coverage caveat.
 */
export interface MatchQualityPoint {
  ...
  matchesCreated: number;
  matchesFilled: number;
  // Confirmation milestones over filled matches (all joined checked in / gave feedback)
  matchesAllCheckedIn: number;
  matchesAllFeedback: number;
  matchesPlayed: number;
  matchesQuality: number;
  // Drop-off branches (partition created together with played)
  matchesCancelled: number;
  matchesMutualCancel: number;
  matchesFellThrough: number;
  matchesPending: number;
  // Why played matches missed the strict bar (overlapping diagnostics)
  playedNoShow: number;
  playedLate: number;
...
export async function getMatchQualityAnalytics(
  startDate: Date,
  endDate: Date
): Promise<MatchQualityPoint[]> {
  try {
    const { data, error } = await supabase.rpc('get_match_quality_analytics', {
      p_start_date: startDate.toISOString().split('T')[0],
      p_end_date: endDate.toISOString().split('T')[0],
    });

    if (error) {
      console.error('Error in getMatchQualityAnalytics:', {
        message: error.message,
        code: error.code,
        details: error.details,
        hint: error.hint,
    ...
    return (data || []).map((row: Record<string, unknown>) => ({
      ...
      matchesCreated: Number(row.matches_created) || 0,
      matchesFilled: Number(row.matches_filled) || 0,
      matchesAllCheckedIn: Number(row.matches_all_checked_in) || 0,
      matchesAllFeedback: Number(row.matches_all_feedback) || 0,
      matchesPlayed: Number(row.matches_played) || 0,
      matchesQuality: Number(row.matches_quality) || 0,
      matchesCancelled: Number(row.matches_cancelled) || 0,
      matchesMutualCancel: Number(row.matches_mutual_cancel) || 0,
      matchesFellThrough: Number(row.matches_fell_through) || 0,
      matchesPending: Number(row.matches_pending) || 0,
      playedNoShow: Number(row.played_no_show) || 0,
...
export interface AutoInviteFunnelPoint {
  /** Bucket key: Monday of the ISO week (bucket='week'), the day (bucket='day'); null in 'total' mode. */
