/**
 * GroupJoinDeepLinkScreen
 *
 * Handles /join/:inviteCode deep links from Universal Links / App Links.
 * This is for group invite links that don't carry a referral code
 * (e.g., shared via QR code or public post).
 *
 * 1. Stores a PendingReferral (with empty code) in AsyncStorage for post-signup handling
 * 2. Sets pending group invite code in DeepLinkContext for immediate consumption
...
export const GroupJoinDeepLinkScreen: React.FC<RootStackScreenProps<'GroupJoinDeepLink'>> = ({
  route,
  navigation,
}) => {
  ...
  const { setPendingGroupInviteCode } = useDeepLink();
  ...
  const handleDeepLink = useCallback(async () => {
    ...
    try {
      ...
      setPendingGroupInviteCode(inviteCode);

      navigation.reset({ index: 0, routes: [{ name: destination }] });
    } catch (error) {
      console.error('Failed to handle group join deep link:', error);
      navigation.reset({ index: 0, routes: [{ name: destination }] });
    }
  }, [inviteCode, navigation, setPendingGroupInviteCode, isSportSelectionComplete]);
