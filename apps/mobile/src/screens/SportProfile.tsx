...
const SportProfile = () => {
  ...
  const refreshProofAndCertificationData = useCallback(async () => {
    ...
    // Update certification status from DB (reflects trigger evaluation)
    if (!certResult.error && certResult.data) {
      const { badge_status, referrals_count, peer_evaluation_average, peer_evaluation_count } =
        certResult.data;

      setReferencesCount(referrals_count || 0);
      setPeerEvaluationAverage(peer_evaluation_average || undefined);
      setPeerEvaluationCount(peer_evaluation_count || 0);

      // Use DB-computed badge_status as the source of truth
      if (badge_status === 'disputed') {
        setCertificationStatus('disputed');
      } else if (badge_status === 'certified') {
