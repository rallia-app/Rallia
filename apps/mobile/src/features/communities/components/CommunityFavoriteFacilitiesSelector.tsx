/**
 * CommunityFavoriteFacilitiesSelector Component
 *
 * A searchable dropdown component for selecting favorite facilities for a community.
 * Shows current favorites and allows adding/removing them (only for moderators/owners).
 * Filters facilities based on the community's sport.
 * Unlike player favorites, communities have no limit on the number of favorites.
 */
...
interface CommunityFavoriteFacilitiesSelectorProps {
  ...
  sportId: string | null;
  /** All sport IDs (tennis + pickleball) for when community has no sport */
  allSportIds?: string[];
...
// No maximum limit for community favorites (unlike player favorites which are limited to 3)
...
export const CommunityFavoriteFacilitiesSelector: React.FC<
  CommunityFavoriteFacilitiesSelectorProps
> = ({
  networkId,
  currentPlayerId,
  sportId,
  ...
  // Fetch community's current favorites
  const {
    favorites,
    ...
    isFavorite,
    canManage,
  } = useCommunityFavoriteFacilities(networkId, currentPlayerId, latitude, longitude);
  ...
  const searchSportIds = useMemo(() => {
    ...
    // Community has no specific sport = both sports
      ...
      {favorites.length === 0 && canManage && !readOnly && (
        <View style={[styles.emptyState, { borderColor: colors.border }]}>
          ...
          <Text style={[styles.emptyStateText, { color: colors.textSecondary }]}>
            {t('community.noFavoriteFacilities')}
          </Text>
          <Text style={[styles.emptyStateHint, { color: colors.textMuted }]}>
            {t('community.addFavoriteFacilitiesHint')}
          </Text>
        </View>
      )}
        ...
        <View>
          ...
          >
            ...
            <TextInput
              ...
              placeholder={
                favorites.length === 0
                  ? t('community.searchFacilityToAdd')
          ...
          {isDropdownOpen && (
            ...
            >
              {filteredResults.length > 0 ? (
                ...
                <View style={styles.noResults}>
                  <Text style={[styles.noResultsText, { color: colors.textMuted }]}>
                    {t('community.typeToSearchFacility')}
                  </Text>
                </View>
              ) : latitude === null || longitude === null ? (
                <View style={styles.noResults}>
                  <Text style={[styles.noResultsText, { color: colors.textMuted }]}>
                    {t('community.locationRequiredForFacilities')}
                  </Text>
                </View>
              ) : null}
            </View>
...
export default CommunityFavoriteFacilitiesSelector;
