...
const CheckInButton: React.FC<CheckInButtonProps> = ({
  ...
  const handleCheckIn = async () => {
    ...
    try {
      // A game with no coordinates has nothing to verify against, so presence
      // is self-declared and no position is needed.
...
export const MatchDetailSheet: React.FC = () => {
  ...
  // Check-in is only available for games with a location (facility or custom)
  const locationAllowsCheckIn =
    match.location_type === 'facility' || match.location_type === 'custom';
  // Geolocation only matters when the game has a point to verify against
  ...
  const playerNeedsCheckIn =
    ...
    (!hasCoordinates || hasLocationPermission);
    ...
    if (playerNeedsCheckIn) {
      return (
        <CheckInButton
          ...
          isAutoGenerated={selectedMatch?.is_auto_generated ?? false}
          requiresLocation={hasCoordinates}
          checkIn={checkIn}
...
L.marker([${resolvedLatitude},${resolvedLongitude}],{icon:icon,interactive:false}).addTo(map);
        ...
        {/* Match Cost Section — cost breakdown */}
        {hasCostData && (
          <Animated.View
