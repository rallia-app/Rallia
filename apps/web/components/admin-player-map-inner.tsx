...
// Facility icon — info blue from design system
const facilityIcon = L.divIcon({
  html: `<span style="width:28px;height:28px;background:${ds.info[500]};box-shadow:0 0 0 2px ${ds.info[300]},0 1px 2px rgba(0,0,0,.1);display:flex;align-items:center;justify-content:center;border-radius:6px;color:white;">
...
// Facility cluster icons — info blue
function createFacilityClusterIcon(cluster: any) {
  const count = cluster.getChildCount();
  const px = count < 10 ? 36 : count < 50 ? 44 : 52;

  return L.divIcon({
    html: `<span style="width:${px}px;height:${px}px;background:${ds.info[500]}cc;display:flex;align-items:center;justify-content:center;border-radius:6px;color:white;font-weight:600;font-size:14px;box-shadow:0 2px 6px rgba(0,0,0,.2);gap:2px;">
  ...
  return (
    ...
    >
      ...
      {facilities.length > 0 && (
        ...
        >
          {facilities.map(f => (
            <Marker key={`f-${f.id}`} position={[f.latitude, f.longitude]} icon={facilityIcon}>
              <Popup>
