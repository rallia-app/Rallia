...
const JWT_AUDIENCE = 'admin_broadcast_unsub';
 ...
 * Verifies the token, flips the preference, then redirects to a confirmation page.
 */
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  if (!secret) {
    return NextResponse.redirect(`${origin}/broadcast/unsubscribe?status=error`);
  }
  if (!token) {
    return NextResponse.redirect(`${origin}/broadcast/unsubscribe?status=error`);
  }
  ...
  return NextResponse.redirect(
    `${origin}/broadcast/unsubscribe?status=${result.ok ? 'success' : 'error'}`
  );
}
