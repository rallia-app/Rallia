...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'broadcastUnsubscribe' });
  return { title: t('successTitle'), robots: { index: false, follow: false } };
}

export default async function BroadcastUnsubscribePage({ params, searchParams }: Props) {
  const { locale } = await params;
  ...
  return (
    <div className="mx-auto max-w-md px-6 py-16">
      <Card className="p-8">
        ...
        <a
          href={`/api/go?to=notificationPreferences&locale=${locale}&src=broadcast_unsubscribe`}
