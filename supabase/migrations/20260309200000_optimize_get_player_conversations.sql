-- Migration: Optimize get_player_conversations
-- This creates a single RPC function that returns all conversation data in one query
-- Replaces the N+1 query pattern with a single database round-trip
...
-- Add helpful comment
COMMENT ON FUNCTION get_player_conversations_optimized IS 'Optimized function to get all conversation previews for a player in a single query. Returns conversations with messages or match-linked chats, ordered by pinned status then last message time.';
