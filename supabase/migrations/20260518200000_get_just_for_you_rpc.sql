...
COMMENT ON FUNCTION public.get_just_for_you(
  ...
  'Single round-trip composer for the Home "Just for you" carousel. Mirrors composeJustForYou (matchScoring + suggestionService + cross-pool dedup) in SQL, returning top p_limit ranked items as JSONB with full MatchWithDetails for match rows. Auth-only; anon callers fall through to the legacy composer at the client.';
