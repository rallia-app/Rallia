-- Enrich the nearby_match_available notification with fields recipients need to
-- judge relevance from the lock screen: when, where, skill bar, and spots left.
--
-- The trigger keeps the same recipient filtering as 20260326000000 (sport, skill,
-- gender) and the same spatial gating; only the payload, title, and body change.
-- Push delivery localizes title/body in the send-notification edge function using
-- these payload fields, so the trigger writes a compact English fallback that
-- mirrors the new layout for the in-app notification list.
...
BEGIN
  ...
  -- skill_level drives the recipient filter; score_value is what we surface to users.
  IF NEW.min_rating_score_id IS NOT NULL THEN
    SELECT rs.skill_level, rs.score_value
