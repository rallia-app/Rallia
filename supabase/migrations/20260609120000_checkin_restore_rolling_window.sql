-- =============================================================================
-- Fix: restore the rolling 4-day window in generate_weekly_matches_for_player
--
-- 20260603130000_checkin_rolling_window rewrote this function to create matches
-- only for the EXACT rolling window today … today+3 (the same 4 days the check-in
-- wizard asks availability for). Two days later, 20260605150000_auto_match_inherit
-- _host_rating reworked the body to add host-rating inheritance but started from
-- the OLD pre-rolling-window version, silently reverting the date logic to "map
-- each player_availability day-of-week onto the current ISO week (Monday-anchored)
-- and keep everything >= today". That regenerates matches for every available
-- weekday from today THROUGH SUNDAY — i.e. days after the 4-day window the wizard
-- actually collected (e.g. check in Monday with Saturday availability → a Saturday
-- match, 5 days out).
--
-- This re-applies the rolling-window iteration (FOR d IN 0..3 over exact dates,
-- idempotent per (sport, exact date)) while KEEPING the host-rating inheritance
-- (min_rating_score_id from the sport's primary rating system) introduced in
-- 20260605150000. Nothing else (facility/hour tiering, conflict guard, dispatch,
-- ACL) changes.
-- =============================================================================
...
COMMENT ON FUNCTION public.generate_weekly_matches_for_player(uuid) IS
  ...
  'check-in wizard collects), hosted by the player. Required level defaults to the '
  'host''s rating in the sport''s primary system (tennis→NTRP, pickleball→DUPR), '
  'NULL when absent. Per day it picks the favorite facility/hour by tier '
