-- =============================================================================
-- Nearby-match notification: align the stored fallback copy with the push
--
-- The in-app notification center renders notification.title/body verbatim (no
-- client re-render), so those stored strings are what the user reads in the list.
-- They were still the old style ("New tennis game nearby" / "… · with Alex").
-- Bring them in line with the personalized push copy from the send-notification
-- renderer:
--   Title: "<Host> wants to play near you"  (or "New game near you")
--   Body:  "<Sport> <when> at <time> at <place>. N spots left, tap to join!"
--
-- Still English-only and computed once per match (not per recipient), so unlike
-- the push it always names the sport and can't vary by the recipient's locale or
-- multi-sport status. Everything else (recipient selection, payload incl.
-- showSport) is unchanged from 20260716130000.
-- =============================================================================
