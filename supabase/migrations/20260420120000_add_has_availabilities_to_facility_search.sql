-- Migration: Add "has availabilities" filter to facility search
-- Description: Adds p_has_availabilities parameter to search_facilities_nearby and
--              search_facilities_nearby_count. When TRUE, only facilities that are
--              considered "bookable" are returned: those with an external_provider_id,
--              or with active local court_slot templates, or with upcoming
--              court_one_time_availability rows.
...
BEGIN
  ...
  ORDER BY sub.is_favorite DESC, sub.distance_meters ASC
  LIMIT p_limit
  OFFSET p_offset;
END;
$$;

-- Recreate count function with p_has_availabilities parameter
...
COMMENT ON FUNCTION search_facilities_nearby_count(UUID[], DOUBLE PRECISION, DOUBLE PRECISION, TEXT, DOUBLE PRECISION, TEXT[], TEXT[], TEXT[], BOOLEAN, BOOLEAN, BOOLEAN) IS
'Get total count of facilities matching search criteria without pagination.
Mirrors search_facilities_nearby filters including p_has_availabilities.';
