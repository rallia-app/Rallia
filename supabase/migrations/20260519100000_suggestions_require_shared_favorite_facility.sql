-- =============================================================================
-- Suggestions: require shared favorite facility
-- =============================================================================
-- Both suggestion paths previously surfaced any opponent×facility pair where
-- the opponent had favorited the facility for this sport — even if the caller
-- had not. That produced suggestions at facilities the caller has never
-- favorited.
--
-- This migration adds a second inner join to `player_favorite_facility` keyed
-- by the caller, so candidate facilities must appear in BOTH players' favorite
-- lists for the chosen sport. Effect:
--
--   • `get_match_suggestions_scored` — Public Matches feed padding
--   • `get_just_for_you`            — Home "Just for you" carousel
--
-- The anon path (`get_match_suggestions_anon`) has no caller and is unchanged.
-- The `+0.30 caller-favorite bonus` inside `score_facility_geo` is left in
-- place to keep the diff minimal; it now always evaluates to 0.30 since the
-- filter is enforced upstream.
--
-- Trade-off: candidate pool shrinks to the intersection of both players'
-- favorites; users with no favorites for the selected sport will see zero
-- suggestions until they add some. Onboarding gates this at min-3.
-- =============================================================================
...
COMMENT ON FUNCTION public.get_match_suggestions_scored(uuid, uuid, integer, double precision, double precision) IS
  'Auth match suggestions. Candidate facilities must appear in BOTH the caller and opponent player_favorite_facility for this sport. score_overlap saturates at /12.0; overlap JSON returns {day, hour}.';

-- =============================================================================
-- RPC: get_just_for_you
-- =============================================================================
-- Single-round-trip composer for the Home "Just for you" carousel. Replaces
-- the legacy 4-7 round-trip pipeline (`getNearbyMatches` → match-detail SELECT
-- → profiles → ratings + `getTopSuggestions` → busy slots → snapshots) with
-- one RPC that:
--
--   1. Scores upcoming matches for the caller (reuses get_upcoming_matches_scored
--      math).
--   2. Scores opponent×facility suggestions (reuses get_match_suggestions_scored
--      math) and expands them into per-hour slots over the next 7 days.
--   3. Cross-pool dedups (drop suggestions for opponents whose match already won
--      a slot in the match pool).
--   4. Merges both pools on a unified score (composer-equivalent), keeps the
--      top p_limit items, and builds the full MatchWithDetails JSONB for
--      match items so the carousel can render — and the detail sheet can open
--      — without any follow-up fetches.
--
-- Public Matches, daily digest, and onboarding stay on the legacy composer;
-- their access patterns differ (30+ items, infinite scroll, per-user iteration).
--
-- Anon callers are NOT supported here. The client wrapper falls back to the
-- legacy composer when p_caller_id is null.
-- =============================================================================
...
BEGIN
  ...
  matchups AS (
    SELECT
      ...
      CASE
        WHEN NOT EXISTS (SELECT 1 FROM public.facility_refresh_log frl WHERE frl.facility_id = f.id) THEN 0.5
        ELSE LEAST(1.0, (
          SELECT COUNT(*)::numeric / 30.0
            FROM public.facility_availability_snapshot fas
           WHERE fas.facility_id  = f.id
...
-- Bump work_mem for the materialized intermediate sets (matches the existing
-- scored RPCs which carry the same scoring CTEs).
...
COMMENT ON FUNCTION public.get_just_for_you(
  ...
  'Single round-trip composer for the Home "Just for you" carousel. Mirrors composeJustForYou (matchScoring + suggestionService + cross-pool dedup) in SQL. Suggestion pool now requires the candidate facility to be favorited by BOTH the caller and the opponent for this sport. Auth-only.';
