-- Migration: Fix certified_via enum casting in certification functions
-- Description: The previous migration (000001) fixed badge_status casts but missed
--              certified_via — which is of type rating_certification_method_enum.
--              Assigning plain text ('proof', 'referral') to an enum column fails
--              because PostgreSQL cannot implicitly cast text to enum in assignments.
-- Fix: Add explicit ::rating_certification_method_enum casts everywhere certified_via
--       is assigned a text literal.
-- Created: 2026-03-19

-- ============================================================================
-- 1. Fix reevaluate_certification_for_player_rating — cast certified_via
-- ============================================================================
    ...
    IF v_should_certify THEN
        ...
        SET 
            ...
            certified_via = CASE 
                ...
                ELSE 'referral'::rating_certification_method_enum 
            END,
            updated_at = NOW()
...
-- ============================================================================
-- 2. Fix check_and_update_certification — cast certified_via assignments
-- ============================================================================
...
BEGIN
    ...
    IF v_should_certify THEN
        ...
        ELSE
            NEW.certified_via := 'referral'::rating_certification_method_enum;
        END IF;
    ELSE
