-- ============================================================================
-- Migration: Exclude blocked players from auth-mode suggestions
-- Created: 2026-05-09
-- Description:
--   Players who have blocked each other (via player_block, in either
--   direction) must never appear in each other's matchup suggestions.
--   Implemented as a hard filter at the opponents-eligibility stage in
--   `get_match_suggestions_scored`.
--
--   Anon mode is unaffected: the caller has no identity, so there's no
--   per-pair block relationship to check.
--
--   No score weights change.
--
--   Note: this only honors `player_block` (user-initiated). Moderation-tier
--   bans (via `player_report.resulting_ban_id`) are handled at the auth
--   layer separately.
-- ============================================================================
...
BEGIN
  ...
  -- Players the caller has blocked (or who have blocked the caller).
  -- Either direction excludes the pair from each other's suggestions.
  blocked_ids AS (
  ...
  opponents AS (
   ...
   WHERE ps.sport_id    = p_sport_id
     ...
     -- New: hard-exclude blocked pairs (either direction).
...
COMMENT ON FUNCTION public.get_match_suggestions_scored(uuid, uuid, integer, double precision, double precision) IS
  ...
  '0.10·activity. Symmetric ±0.5 rating filter and player_block exclusion '
  '(both directions) at the opponent-eligibility stage.';
