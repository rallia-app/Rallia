-- Migration: Exclude expired matches from upcoming tab
-- Description: Expired matches (start_time passed but match not full) should not
--              appear in the upcoming tab, even for creators and joined players.
--              Only truly ongoing matches (start_time passed AND match is full)
--              remain visible in upcoming for confirmed participants and creators.
...
COMMENT ON FUNCTION get_player_matches(UUID, TEXT, UUID, INT, INT, TEXT) IS 'Get matches for a player with optional status filtering. Upcoming shows matches where start_time has not been reached (any participant status) or ongoing full matches where user has joined status or is creator. Expired matches (start_time passed, not full) are excluded from upcoming and appear in past instead.';
