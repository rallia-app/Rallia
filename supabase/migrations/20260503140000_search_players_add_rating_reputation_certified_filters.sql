-- Migration: Add rating, reputation, and certified filters to search_players_nearby RPC
-- Description: Adds p_rating_score_ids (multi-select), p_reputation_tier (single-select),
--              and p_certified_only (boolean) filter parameters to the player search RPC.
--              All new params have defaults so existing callers continue to work.
...
COMMENT ON FUNCTION public.search_players_nearby IS
  'Server-side player directory search. Returns a paginated, filtered, sorted slice with rating, reputation, and online status. Supports multi-select rating score IDs, reputation tier, and certified-only filters. Distance in meters via PostGIS when p_latitude/p_longitude provided.';
