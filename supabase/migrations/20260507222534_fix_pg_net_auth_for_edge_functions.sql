-- Switches every pg_net caller (cron jobs + trigger/RPC functions) from sending
-- the legacy anon JWT in Authorization-only to sending the service_role secret
-- key in BOTH `apikey:` and `Authorization: Bearer ...` headers.
--
-- Reason (Why): Supabase's gateway no longer accepts publishable/secret keys
-- in the Authorization header alone. Calls without an `apikey:` header started
-- 401'ing once the project's legacy anon JWT was disabled.
-- See https://supabase.com/docs/guides/getting-started/api-keys (Known limitations).

-- ---------------------------------------------------------------------------
-- 1. Cron jobs — six active jobs, swap anon_key -> service_role_key + apikey
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. Trigger / RPC functions — replace anon_key vault read + Authorization-only
--    header with service_role_key + apikey + Authorization headers.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF functions_url IS NULL OR service_key IS NULL THEN
    RAISE WARNING 'Cannot send invitation email: vault secrets missing (url: %, key: %)',
...
BEGIN
  ...
  IF functions_url IS NULL OR service_key IS NULL THEN
    RAISE WARNING 'Cannot dispatch notification: Vault secrets not configured (url: %, key: %)',
      CASE WHEN functions_url IS NULL THEN 'missing' ELSE 'ok' END,
      CASE WHEN service_key IS NULL THEN 'missing' ELSE 'ok' END;
    RETURN NEW;
  END IF;
...
BEGIN
  ...
  IF functions_url IS NULL OR service_key IS NULL THEN
    RAISE WARNING '[notify_admin_new_feedback] Vault secrets not configured (url: %, key: %)',
      CASE WHEN functions_url IS NULL THEN 'missing' ELSE 'ok' END,
      CASE WHEN service_key IS NULL THEN 'missing' ELSE 'ok' END;
    RETURN NEW;
  END IF;
...
BEGIN
  ...
  IF functions_url IS NULL OR service_key IS NULL THEN
    RAISE WARNING 'Cannot send match interest email: vault secrets missing (url: %, key: %)',
      CASE WHEN functions_url IS NULL THEN 'missing' ELSE 'ok' END,
      CASE WHEN service_key IS NULL THEN 'missing' ELSE 'ok' END;
    RETURN NEW;
  END IF;
...
BEGIN
  ...
  IF functions_url IS NULL OR service_key IS NULL THEN
    RAISE WARNING 'Cannot send welcome email: vault secrets missing (url: %, key: %)',
      CASE WHEN functions_url IS NULL THEN 'missing' ELSE 'ok' END,
      CASE WHEN service_key IS NULL THEN 'missing' ELSE 'ok' END;
    RETURN NEW;
  END IF;
...
-- These two were also broken: they were reading from current_setting() GUCs
-- that were never populated, so they fell back to a placeholder URL. Rewrite
-- them to use vault like the others.
