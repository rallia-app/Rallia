-- ============================================
-- Forward-fix: invite-join disqualified gate + partner index
-- ============================================
-- 20260612150000 / 20260612160200 shipped tournament_join_via_invite WITHOUT
-- the "disqualified is terminal" gate that 20260612140000 added to
-- tournament_register. Result: an organizer-removed player (status
-- 'disqualified') could re-tap the still-valid invite link and silently
-- reactivate their registration, undoing the removal.
--
-- Those two migrations were already applied to staging, so the fix can't go
-- back into their (immutable) bodies — this forward migration re-creates the
-- function with the gate and adds the partner_user_id index (a hot predicate
-- for the partner-aware RLS / myTournaments OR-filter that had no index).
...
COMMENT ON FUNCTION public.tournament_join_via_invite(text, uuid)
    IS 'Registers the caller via a tournament invite token. Bypasses registration_mode; idempotent for already-active members (captain or partner); organizer-removed (disqualified) players stay blocked. Doubles require p_partner_id.';

-- partner_user_id became a hot predicate (myTournaments OR-filter, treg_select,
-- is_tournament_registrant); without this the OR forces a seq scan.
CREATE INDEX IF NOT EXISTS tournament_registrations_partner_idx
    ON tournament_registrations(partner_user_id)
