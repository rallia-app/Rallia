-- Migration: Include reputation + online status in search_players_nearby RPC
-- Description: Eliminates N+1 queries (30+ per page load) by returning
--              reputation_tier, reputation_score, reputation_is_public, and
--              last_seen_at directly from the player search RPC.
...
COMMENT ON FUNCTION public.search_players_nearby IS
  'Server-side player directory search. Returns a paginated, filtered, sorted slice with rating, reputation, and online status. Distance in meters via PostGIS when p_latitude/p_longitude provided.';
