-- Weekly-cohort view for the admin invite funnel: adds p_bucket ('total' = current
-- single-row-per-sport behaviour, 'week' = one row per ISO week per sport) and a
-- bucket_start output column (NULL in total mode, Monday of the week otherwise).
-- Each population is bucketed by its own anchor: created/invited by created_at,
-- requests by requested_at. Return table changes, so drop + recreate; the new
-- param is defaulted, so existing 4-named-arg PostgREST calls keep working.
...
CREATE OR REPLACE FUNCTION public.get_auto_invite_funnel(
  ...
  p_bucket       text DEFAULT 'total'   -- 'total' | 'week'
) RETURNS TABLE (
  bucket_start          date,
  sport_id              uuid,
  sport_name            text,
...
DECLARE
  ...
  v_weekly boolean := (p_bucket = 'week');
BEGIN
  RETURN QUERY
  WITH created AS (
    SELECT
      CASE WHEN v_weekly THEN date_trunc('week', m.created_at)::date END AS bucket_start,
      m.sport_id,
      COUNT(*) AS n
    FROM match m
  ...
  invited AS (
    SELECT
      CASE WHEN v_weekly THEN date_trunc('week', mp.created_at)::date END AS bucket_start,
      m.sport_id,
      mp.status,
  ...
  requests AS (
    SELECT
      CASE WHEN v_weekly THEN date_trunc('week', mp.requested_at)::date END AS bucket_start,
      m.sport_id,
      COUNT(*)                                          AS total,
...
COMMENT ON FUNCTION public.get_auto_invite_funnel IS
  'Per-sport invitation funnel anchored on invite-sent time, for matches of source p_is_auto (true=auto, false=human-created, NULL=all). p_bucket=''week'' splits every population into ISO-week cohorts (bucket_start = Monday; created/invited by created_at, requests by requested_at); ''total'' returns one row per sport with bucket_start NULL. Rates over the settled cohort (invites older than p_settle_hours); younger invites are in_flight. Includes why-invites-die data: decline_reasons jsonb, median_response_hours, no_response_expired.';
