-- =============================================================================
-- Suggestion scoring: add score_bookability from the availability snapshot
-- (Step 4 of the SWR availability pipeline)
--
-- Folds the snapshot into facility affinity as a soft signal. Facilities
-- with more confirmed-bookable slots in the next 3 days rank higher; cold-
-- start facilities (no provider, or never refreshed) get a neutral 0.5 so
-- they neither gain nor lose ground while coverage ramps up.
--
-- score_bookability:
--   - 0.5 if facility has no external provider (org-managed locals)
--   - 0.5 if facility has a provider but no facility_refresh_log row yet
--   - LEAST(1.0, COUNT(*) / 30.0) otherwise, counted over the next 3 days
--     against rows with is_available = TRUE
--
-- Facility-affinity rebalance (each set sums to 1.0):
--   AUTH:  shared-fav 0.40 → 0.30 · caller-dist 0.30 → 0.25 ·
--          opp-dist 0.30 → 0.25 · bookability → 0.20
--   ANON:  caller-dist 0.60 → 0.50 · opp-dist 0.40 → 0.30 · bookability → 0.20
--
-- Everything else (opponent CTEs, responsiveness/activity signals, block
-- filter, tier masking, rating filter, location override) is identical to
-- 20260510160000.
-- =============================================================================
...
BEGIN
  ...
  matchups AS (
    SELECT
      ...
      -- Snapshot-derived bookability. 0.5 neutral when the facility has no
      -- external provider or has never been refreshed; real ratio otherwise.
      CASE
...
COMMENT ON FUNCTION public.get_match_suggestions_anon(uuid, double precision, double precision, integer, integer) IS
  'Anon match suggestions. facility_affinity now = 0.50·caller-dist + 0.30·opp-dist + 0.20·bookability (from facility_availability_snapshot). Cold-start facilities get neutral 0.5 bookability.';

COMMENT ON FUNCTION public.get_match_suggestions_scored(uuid, uuid, integer, double precision, double precision) IS
  'Auth match suggestions. facility_affinity now = 0.30·shared-fav + 0.25·caller-dist + 0.25·opp-dist + 0.20·bookability (from facility_availability_snapshot). Cold-start facilities get neutral 0.5 bookability.';
