-- Migration: Simplify player matches filter options
-- Description: Add 'waiting' filter (merges pending/requested/waitlisted),
--              rename 'played' → 'completed', 'expired' → 'unfilled',
--              remove 'ready_to_play' and 'as_participant' filters.
--              Old individual filters (pending, requested, waitlisted) still work in SQL
...
COMMENT ON FUNCTION get_player_matches(UUID, TEXT, UUID, INT, INT, TEXT) IS 'Get matches for a player with optional status filtering. Upcoming excludes expired matches (start passed, not full). Past includes expired matches as soon as start_time passes. Supports filters: all, hosting, confirmed, waiting, needs_players (upcoming), feedback_needed, completed, hosted, unfilled, cancelled (past). Legacy filters pending, requested, waitlisted, played, expired still work.';
