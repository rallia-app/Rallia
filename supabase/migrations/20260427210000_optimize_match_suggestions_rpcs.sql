-- Optimize get_match_suggestions_anon and get_match_suggestions_scored to reduce Disk IO.
-- Three changes per function:
--   1. SET work_mem = '32MB' to keep the final ORDER BY/sort in memory (was spilling to temp files).
--   2. Cap the opponents CTE early with ORDER BY ST_Distance + LIMIT (safety valve for popular cities).
--   3. Defer the per-opponent availability json_agg subquery until after the final LIMIT, so it only
--      runs for surviving rows. Implemented via a MATERIALIZED `ranked` CTE.
-- Behavior is unchanged: same columns, same ordering, same scores.
