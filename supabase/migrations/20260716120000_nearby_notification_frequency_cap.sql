-- =============================================================================
-- Nearby-match notification: cap at 3 per rolling 7 days per player
--
-- Defense-in-depth on top of the 5 km radius cap (20260715130000). Even within
-- 5 km, a dense pocket can still over-notify a player. Exclude any recipient who
-- has already received 3+ nearby_match_available notifications in the trailing
-- 7 days. Rolling window (not calendar week) to avoid a Monday flood.
--
-- Only the recipient filter changes; radius, rating gate, exclusions,
-- sport/gender filters and payload are unchanged.
-- =============================================================================

-- Keep the per-recipient count cheap as the table grows.
CREATE INDEX IF NOT EXISTS idx_notification_nearby_user_created
  ON public.notification (user_id, created_at)
  WHERE type = 'nearby_match_available';
