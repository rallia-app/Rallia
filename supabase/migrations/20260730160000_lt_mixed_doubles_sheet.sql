-- ============================================================================
-- Leagues — mixed_doubles sessions pair 2v2, matching the tournament rule
-- ============================================================================
-- entry_format has three labels: singles, doubles, mixed_doubles. Tournaments
-- branch on entry_format <> 'singles', so mixed doubles gets the full partner
-- flow (no gender enforcement anywhere — it is doubles under another label).
-- 20260730150000 branched on = 'doubles', so a league season configured with
-- formatsAllowed ["mixed_doubles"] silently fell through to the SINGLES path
-- and produced 1v1 sheets.
--
-- Same body as 20260730150000 with the branch, the four-player floor and the
-- mutual-pair scan keyed on <> 'singles', and match rows stamped with the
-- session's actual format instead of the literal 'doubles'.
-- ============================================================================
...
'BY_RANK pairing for a published session, singles or doubles/mixed per the session''s
