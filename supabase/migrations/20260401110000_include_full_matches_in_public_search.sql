-- Migration: Include full matches in public match search results
-- Previously, search_public_matches excluded matches where filled_spots >= total_spots.
-- Full matches should still appear in public games (users can see them, just can't join).
-- The count function (search_public_matches_count) already included full matches,
-- causing a mismatch between the displayed count and actual results.
...
COMMENT ON FUNCTION search_public_matches IS
'Search public matches with comprehensive filters. Full matches are included in results.';
