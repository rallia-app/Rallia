-- =============================================================================
-- Weekly check-in plan preview: decouple proposal count from goal gap
--
-- The wizard preview was capped at (weekly_goal - committed_matches), so a
-- player who only needed 1 more game saw a single proposal with no alternates.
-- Preview now surfaces up to 10 ranked options across declared hours so the
-- player can reject some and pick others. The autonomous generator (no
-- p_max_proposals) keeps the goal-net cap unchanged.
--
-- Proposal keys now include start_hour so multiple slots on the same sport/day
-- are addressable in the client toggle state.
-- =============================================================================
...
CREATE OR REPLACE FUNCTION public.plan_weekly_matches_for_player(
  ...
  p_max_proposals int DEFAULT NULL
)
RETURNS TABLE (
  sport_id            uuid,
  sport_name          varchar,
...
BEGIN
  ...
  IF p_max_proposals IS NOT NULL THEN
    -- Wizard preview: offer a generous menu of options, not just the goal gap.
    v_to_create := GREATEST(p_max_proposals, 0);
  ELSE
    -- Autonomous generator: still net against the weekly objective.
...
COMMENT ON FUNCTION public.plan_weekly_matches_for_player(uuid, jsonb, int, int) IS
  'PURE planner (zero writes). Rolling window today…today+3, soonest-first across '
  'days/sports/hours. p_max_proposals set (wizard preview) → up to that many '
  'ranked options regardless of goal gap; NULL (autonomous generator) → capped '
  'at weekly goal minus committed upcoming games.';

REVOKE ALL ON FUNCTION public.plan_weekly_matches_for_player(uuid, jsonb, int, int) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.plan_weekly_matches_for_player(uuid, jsonb, int, int) TO service_role;


-- get_checkin_match_plan: preview up to 10 proposals; unique keys per hour.
...
BEGIN
  ...
  FOR v_plan IN
    SELECT * FROM public.plan_weekly_matches_for_player(
      v_player_id, p_slots, v_goal, v_preview_cap
    )
  LOOP
    SELECT COALESCE(
...
COMMENT ON FUNCTION public.get_checkin_match_plan(jsonb, smallint, text, int) IS
  'Check-in wizard plan PREVIEW: up to 10 ranked proposals from '
  'plan_weekly_matches_for_player (independent of goal gap). committed_count '
  'still reflects games already lined up for the progress bar.';
