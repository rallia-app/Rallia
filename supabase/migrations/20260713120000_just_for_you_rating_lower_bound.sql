-- ----------------------------------------------------------------------------
-- get_just_for_you — bound the match pool to the caller's rating band
--
-- Previously the candidate-match filter only capped the UPPER side of a match's
-- minimum rating (min_rating <= caller + 0.5) with no lower bound, so games
-- whose minimum rating sat BELOW the caller's rating still surfaced in the
-- "Just for you" carousel. Add the lower bound so a match's minimum rating must
-- fall in [caller_rating, caller_rating + 0.5] — at the caller's level or up to
-- half a point above. Open matches (no minimum) and anon callers (no rating)
-- are unaffected.
--
-- Only the match-pool WHERE clause changes; the rest of the function body is a
-- verbatim reproduction of the live definition (20260609220000).
-- ----------------------------------------------------------------------------
  ...
  candidate_matches AS (
    ...
    WHERE m.visibility = 'public'
      ...
      AND (
        ...
        OR (mr.value >= v_caller_rating_value AND mr.value <= v_caller_rating_value + 0.5)
      )
  ),
