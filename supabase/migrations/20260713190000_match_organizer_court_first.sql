-- ============================================================================
-- Chat Match Organizer — engine v3: court-availability-first suggestions
-- ============================================================================
-- Prioritize REAL upcoming open courts near both players over speculative
-- "usually free" configurations. Two candidate sources, unified + rescored:
--
--   Source A (court-driven): every upcoming open-court snapshot slot for the
--     sport at a facility that is EITHER favorited (a favorite is travel
--     willingness) OR within every located player's max_travel_distance, and
--     where >=1 player has recurring availability at that hour. Always bookable.
--   Source B (speculative): recurring mutual-overlap hours x favorited
--     facilities with no live court -> the "usually free" floor. Only emitted
--     for (facility,date,hour) not already covered by a real court.
--
-- Scoring guarantees every real open court outranks every speculative option
-- (+1000), then within courts: both-free (+200) > one-free, favorite boost,
-- sooner, nearer. Diversified to the best hour per (facility, date).
--
-- Return signature is unchanged from v2 -> CREATE OR REPLACE (no client change).
-- ============================================================================
...
COMMENT ON FUNCTION public.match_organizer_options(uuid[], uuid, int, int) IS
  'Ranked time/place options for N players + sport, court-availability-first: real upcoming open courts near both players (>=1 free) always outrank speculative usually-free favorited-park overlap slots. Generic (no tournament dependency).';
