-- ============================================================================
-- Drop the "compatible players next week" tier from the unfilled-host recovery
--
-- The strongest copy tier told a host whose game just failed to fill that we
-- had "already spotted N compatible players for the same slot next week". That
-- reads as a promise the number cannot support: those same players were
-- reachable this week too and did not join, so counting them proves nothing
-- about next week. It is the same false confidence the badge honesty pass
-- removed from the invite list, in message form.
--
-- The notification keeps the two honest tiers: name the open games actually
-- near the host in the next 4 days, otherwise say something plain. The
-- get_auto_invite_candidates_for_slot call goes away with it, which also drops
-- the most expensive part of the sweep.
--
-- Body copied verbatim from 20260805150100 with only that tier removed.
-- ============================================================================
...
BEGIN
  ...
  LOOP
    ...
    -- Open public games of the same sport in the next 4 days
...
COMMENT ON FUNCTION public.send_unfilled_host_recovery() IS
  'Momentum item 7: recovers hosts whose game started unfilled by pointing at nearby open games. Hourly pg_cron.';
