-- Migration: Inline availability slots into search_facilities_nearby
-- Description: Adds an `availability_slots JSONB` column to the RPC return
--              shape, populated via a LATERAL subquery on
--              facility_availability_snapshot for the paginated rows only.
--              Eliminates the per-card useCourtAvailability fetch that was
--              causing scroll jank in the mobile facilities directory: the
--              snapshot is now the single source of truth for list-view
--              slots, and the join runs at most p_limit times per page.
--
--              Slots are filtered to:
--                - facility match
--                - sport_id = ANY(p_sport_ids) (legacy NULL rows excluded,
--                  matching the established p_has_open_slots scoping)
--                - is_available = TRUE
--                - slot_start > now()
--              Capped at 60 slots per facility (a few days × multiple courts)
...
BEGIN
  ...
  -- (facility_id, sport_id, slot_start) lookup.
  LEFT JOIN LATERAL (
    SELECT jsonb_agg(
      jsonb_build_object(
    ...
    FROM (
      ...
      WHERE facility_id = sub.id
        AND is_available = TRUE
        AND slot_start > now()
        AND sport_id = ANY(p_sport_ids)
      ORDER BY slot_start, external_court_id
      LIMIT 60
    ) fas
  ) slots ON TRUE
...
COMMENT ON FUNCTION search_facilities_nearby IS
  ...
  'future-only, available, capped at 60 per facility) so the mobile list '
  'view does not need per-card availability queries. p_has_availabilities '
  'filters to facilities that *could* be booked; p_has_open_slots is '
  'stricter and requires a current row in facility_availability_snapshot.';
