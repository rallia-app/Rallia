-- =============================================================================
-- Nearby-match notification: cap the notification radius at 5 km
--
-- The "nearby" test used the player's full max_travel_distance as the radius
-- (median 20 km, up to 50 km). In a metro that width, a wide-radius player was
-- eligible for nearly every public match at their rating, producing a push
-- storm (heavy users averaged ~91 nearby pushes over 5 weeks; per-push CTR
-- collapsed to ~2.75%). max_travel_distance means "how far I'd travel for a
-- match I choose", not "how far away a match can be before you stop pinging me".
--
-- Fix: cap the notification radius at LEAST(max_travel_distance, 5) km. The
-- favorite-facility branch is unchanged, so a player still hears about matches
-- at a facility they favorited regardless of distance. Rating gate, exclusions,
-- sport/gender filters and payload are all unchanged.
-- =============================================================================
