-- ============================================
-- Fix: tournament_generate_bracket TEMP TABLE collides on multiple calls per txn
-- ============================================
-- The bracket generator uses an internal `_gen_matches` temp table to map
-- (round_number, match_position) → match_id while wiring next_match_id
-- pointers across rounds. It was declared:
--
--     CREATE TEMP TABLE _gen_matches (...) ON COMMIT DROP;
--
-- ON COMMIT DROP only fires when the surrounding transaction commits, so
-- two generate_bracket calls inside the same transaction collide on the
-- second CREATE. This shows up in batch test scripts and would also hit
-- any future code path that generates multiple brackets atomically.
--
-- Fix: switch to ON COMMIT DROP + IF NOT EXISTS, and TRUNCATE on entry so
-- prior contents don't bleed into the new run. The body is otherwise
-- untouched from 170008.
-- ============================================
...
BEGIN
    ...
    -- Robust against multiple calls per transaction: don't fail on second
    -- CREATE, and start each invocation from an empty mapping.
    CREATE TEMP TABLE IF NOT EXISTS _gen_matches (
        round_number   smallint NOT NULL,
        match_position smallint NOT NULL,
        match_id       uuid NOT NULL,
        PRIMARY KEY (round_number, match_position)
    ) ON COMMIT DROP;
    TRUNCATE _gen_matches;
