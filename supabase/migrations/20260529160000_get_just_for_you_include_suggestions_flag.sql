-- ════════════════════════════════════════════════════════════════════════
-- get_just_for_you — add p_include_suggestions flag (matches-only carousel)
-- ════════════════════════════════════════════════════════════════════════
--
-- Adds a p_include_suggestions boolean param (DEFAULT true). When false, the
-- suggestion pool is pruned at its root CTEs (shared_fac_opponents +
-- caller_opp_overlap gated to empty), so the carousel returns matches only and
-- skips the opponent scan / slot fan-out / bookability work entirely. The
-- signed-in client now passes false — Home "Just for you" is matches-only,
-- matching the signed-out path (get_nearby_public_matches). DEFAULT true keeps
-- the function backward-compatible for any caller that omits the arg (e.g. an
-- older app build still in flight during deploy).
--
-- The function BODY below is the migration 20260526180000 (fix #3) body
-- VERBATIM, except (a) the new trailing param and (b) the two
-- `AND p_include_suggestions` guards. All scoring math / JSONB payloads / merge
-- are unchanged, so suggestions-on output is byte-identical to before. Because
-- the signature changes (7 → 8 args) we DROP the old overload and recreate,
-- which clears proconfig — so jit=off (migration 20260529120000) and work_mem
-- are re-applied at the bottom.
--
-- ── Original fix-#3 header (body provenance) ──────────────────────────────
-- ════════════════════════════════════════════════════════════════════════
-- Optimize get_just_for_you — fix #3: compute facility bookability ONCE
-- ════════════════════════════════════════════════════════════════════════
--
-- Follow-up to 20260526170000 (fixes #1 + #2). Only the suggestion-pool's
-- facility bookability score changes; scoring math, the match pool, JSONB
-- payloads, and final merge are UNCHANGED — results are identical.
--
--   #3 The `matchups.score_bookability` term used a correlated COUNT subquery
--      over `facility_availability_snapshot`, evaluated once PER matchup row
--      (≈373× for a heavy caller). Because that table churns constantly from
--      the refresh worker its visibility map is always stale, so each call
--      degraded into heap fetches — ~50,000 heap fetches / ~34k buffers per
--      request just for bookability, the dominant cost of the suggestion pool.
--
--      We now compute the per-facility bookable score ONCE, in a MATERIALIZED
--      CTE (`facility_bookable`) restricted to the caller's favorited
--      facilities (<=21), and LEFT JOIN it into `matchups`. The snapshot is
--      scanned a single time instead of per-row: bookability buffer traffic
--      drops from ~34k to ~600 (≈50× fewer heap fetches). Result-equivalent —
--      verified identical per-facility score_bookability checksum on prod.
--
--      MATERIALIZED is required: without it the planner inlines the CTE and
--      re-runs the aggregate per matchup row (measured even worse). The inner
--      scan is restricted to caller facilities so it's a single grouped pass.
--
-- NOTE: the wall-clock win is partly masked by autovacuum churn on the
-- snapshot table (timings are noisy run-to-run); the reliable signal is the
-- ~50× reduction in heap fetches / buffer traffic, which matters most under
-- production IO pressure. A persisted rollup maintained by the refresh worker
-- remains a future option if bookability ever needs to be a pure O(1) lookup.
-- ════════════════════════════════════════════════════════════════════════

-- Signature changes (adds p_include_suggestions), so the old 7-arg overload
-- must go — otherwise both coexist and 7-arg calls would keep hitting the old
-- body. After the drop, callers that omit the new arg fall through to this
-- function via its DEFAULT true (so old app builds keep working).
DROP FUNCTION IF EXISTS public.get_just_for_you(
  uuid, uuid, double precision, double precision, double precision, text, integer
);

CREATE OR REPLACE FUNCTION public.get_just_for_you(
  p_caller_id           uuid,
  p_sport_id            uuid,
  ...
  -- When false, the suggestion pool is pruned entirely (matches-only carousel).
...
LANGUAGE plpgsql
-- VOLATILE (the default) because the body calls random() for jitter and we
-- want a fresh shuffle on every invocation. Not STABLE.
VOLATILE
SECURITY DEFINER
AS $function$
DECLARE
...
-- Re-apply function-level settings — DROP+CREATE for the new signature clears
-- the proconfig that ALTER FUNCTION set on the old 7-arg overload:
--   • work_mem: headroom for the materialized intermediate sets.
--   • jit=off:  this single ~15-CTE statement's cost trips jit_above_cost, and
--     on a fresh pooled backend the LLVM compile dwarfs runtime at Rallia's
--     scale (tipped cold logins over the 8s timeout — migration 20260529120000).
ALTER FUNCTION public.get_just_for_you(
  uuid, uuid, double precision, double precision, double precision, text, integer, boolean
) SET work_mem = '32MB';
...
GRANT EXECUTE ON FUNCTION public.get_just_for_you(
  uuid, uuid, double precision, double precision, double precision, text, integer, boolean
) TO authenticated;

COMMENT ON FUNCTION public.get_just_for_you(
  uuid, uuid, double precision, double precision, double precision, text, integer, boolean
) IS
  'Single round-trip composer for the Home "Just for you" carousel. Mirrors composeJustForYou (matchScoring + suggestionService + cross-pool dedup) in SQL. p_include_suggestions (DEFAULT true) gates the suggestion pool: when false the carousel is matches-only and the opponent scan / slot fan-out / bookability work is skipped. Auth-only. Suggestion pool requires the candidate facility to be favorited by BOTH caller and opponent; pre-filters opponents to the shared-favorite-facility set, computes caller-opponent overlap once (20260526170000) and per-facility bookability once in a MATERIALIZED CTE (20260526180000).';
