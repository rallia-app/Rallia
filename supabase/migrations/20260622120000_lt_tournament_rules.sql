-- ============================================
-- Leagues & Tournaments — tournament rules text
-- ============================================
-- Adds a free-text `rules` field to tournaments (organizer-authored rules
-- shown on the tournament sheet) and threads it through tournament_update
-- with the same editable-by-state gate as `description`.
-- ============================================

ALTER TABLE public.tournaments ADD COLUMN IF NOT EXISTS rules text;

-- Recreate tournament_update with `rules` added to the per-field state gate
-- and the UPDATE set. Body otherwise identical to 20260612090000.
