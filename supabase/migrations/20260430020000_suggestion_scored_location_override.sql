-- Allow callers to pass an explicit location to get_match_suggestions_scored.
-- When p_lat / p_lng are provided they override the player's stored home location
-- for all proximity calculations (facility distance, opponent distance).
-- This lets the mobile client honour the user's "home vs GPS" toggle without
...
BEGIN
  ...
  -- Override stored location when the caller provides explicit coordinates
  -- (e.g. GPS mode selected in the app instead of saved home address).
