-- ============================================================================
-- Parameterize min_events_for_public threshold
-- ============================================================================
-- The `is_public` calculation in recalculate_player_reputation() was hardcoded
-- to `>= 10`, duplicating the value defined in reputationConfig.ts as
-- MIN_EVENTS_FOR_PUBLIC. This adds a parameter so the threshold flows through
-- from a single source of truth rather than being scattered across SQL and TS.
--
-- The default (10) matches the current behavior so existing callers (the
-- AFTER INSERT trigger, edge functions) continue to work unchanged.
-- ============================================================================

CREATE OR REPLACE FUNCTION recalculate_player_reputation(
  target_player_id UUID,
  apply_decay BOOLEAN DEFAULT false,
  min_events_for_public INT DEFAULT 10
