-- ============================================================================
-- Inbox RPCs: expose last_message_type + a trimmed metadata object
--
-- Base text: 20260720140100_conversation_rpcs_announcement.sql (latest),
-- transformed mechanically so nothing else drifts.
--
-- Why: structured cards store their preview line as literal `content` written
-- at post time. A card posted by the server has no single locale to write in,
-- so it ended up bilingual ("Suggestions d'heures · Suggested times") and every
-- reader saw both languages. The inbox only ever received `last_message_content`
-- and the sender's name, so the client could not tell a card from a chat message
-- and had no way to localize it.
--
-- These two columns let the conversation list render its own localized preview
-- per viewer and fall back to `content` for anything it does not recognize.
--
-- `last_message_meta` is deliberately a trimmed projection, not `m.metadata`:
-- a match_organizer card carries up to 10 option objects, and shipping those for
-- every conversation row would bloat the inbox payload for no reason.
-- ============================================================================

-- ============================================================================

-- ============================================================================
-- 1. get_player_conversations_optimized
-- ============================================================================
...
BEGIN
  ...
  last_messages AS (
    SELECT DISTINCT ON (m.conversation_id)
      ...
      jsonb_strip_nulls(jsonb_build_object(
        ...
        'no_overlap',  m.metadata->'no_overlap',
        'system_note', m.metadata->'system_note',
  ...
  SELECT
    ...
    ti.t_id AS tournament_id,
    ti.t_name AS tournament_name,
    ti.t_status AS tournament_status,
    ti.t_sport_name AS tournament_sport_name
  FROM conversation c
  ...
  LEFT JOIN tournament_info ti ON ti.conversation_id = c.id
  WHERE (
      lm.message_id IS NOT NULL
      OR c.match_id IS NOT NULL
      OR c.tournament_id IS NOT NULL
      OR c.conversation_type = 'announcement'
    )
    AND (
      p_sport_id IS NULL
      OR mi.match_sport_id = p_sport_id
      OR ni.network_sport_id = p_sport_id
      OR ti.t_sport_id = p_sport_id
      OR c.sport_id = p_sport_id
      OR (
        mi.match_sport_id IS NULL
        AND ni.network_sport_id IS NULL
        AND ti.t_sport_id IS NULL
...
COMMENT ON FUNCTION get_player_conversations_optimized IS 'Optimized function to get all conversation previews for a player in a single query. Supports optional sport filtering. Returns conversations with messages, match-linked chats, tournament-linked chats, or sport announcement channels, ordered by pinned status then last message time.';

-- ============================================================================
-- 2. get_player_conversations_filtered
-- ============================================================================
...
BEGIN
  ...
  full_results AS (
    SELECT
      ...
      ti.t_status AS tournament_status,
      ti.t_sport_name AS tournament_sport_name,
      ti.t_sport_id AS tournament_sport_id
    FROM conversation c
    JOIN player_conversations pc ON pc.conversation_id = c.id
    LEFT JOIN last_messages lm ON lm.conversation_id = c.id
    LEFT JOIN participant_counts pcount ON pcount.conversation_id = c.id
  ...
  SELECT
    ...
    fr.last_message_type,
    fr.last_message_meta,
    fr.is_pinned,
    fr.is_muted,
    fr.is_archived,
    fr.last_read_at,
    ...
    fr.tournament_id,
    fr.tournament_name,
    fr.tournament_status,
    fr.tournament_sport_name
  ...
  WHERE
    CASE p_filter
      ...
      WHEN 'tournament' THEN fr.conversation_type = 'tournament'
      WHEN 'announcement' THEN fr.conversation_type = 'announcement'
      ELSE true
    END
    AND (
      p_search = ''
      ...
      OR fr.tournament_name ILIKE '%' || p_search || '%'
    )
  ORDER BY
    fr.is_pinned DESC,
...
COMMENT ON FUNCTION get_player_conversations_filtered IS
  'Filtered + paginated conversation previews for the chat inbox. Supports type filter (incl. tournament and announcement), search, pagination, and sport filtering. Excludes archived conversations.';
