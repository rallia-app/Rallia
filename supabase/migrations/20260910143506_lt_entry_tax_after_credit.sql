-- ============================================================================
-- Entry tax follows the credit.
--
-- 20260910020252 computed the entry tax on the LISTED entry, before any
-- referral credit came off, and left the question open. Decided 2026-09-10:
-- a credit Rallia grants and nobody reimburses is a price reduction, so the
-- consideration for the entry is what the player actually pays. A player
-- using the 10 $ credit on a 15 $ entry pays 5 $, and the tax inside that is
-- 0,65 $, not 1,95 $. A fully covered registration carries no tax.
--
-- Only the two begin RPCs change, and only after the credit is reserved:
-- the entry-side amount paid is entry charge minus credit (the floor keeps
-- fee + fee tax out of the credit, so the whole draw came off the entry), and
-- the tax is carved out of that amount in both modes. With no credit the
-- figure is exactly what it was. Return shapes are unchanged, so no DROP.
--
-- The quotes still preview the pre-credit tax next to credit_applicable_cents;
-- the ledger row written here is the figure that counts. Bodies copied from
-- 20260910020252, the latest.
-- ============================================================================
...
BEGIN
    ...
    -- A credit Rallia grants and nobody reimburses is a price reduction, so
    -- the entry tax rides on what the player actually pays for the entry, not
    -- on the listed price. The floor keeps fee + fee tax out of the credit, so
    -- everything drawn came off the entry side. Carved out of the paid amount
    -- in both modes: a fully covered registration carries no tax at all.
    IF v_credit > 0 AND v_t.entry_tax_mode <> 'none' THEN
        v_entry_tax := public.compute_entry_tax_cents(
            GREATEST(v_entry_chg - v_credit, 0), 'included');
    END IF;
...
BEGIN
    ...
    -- everything drawn came off the entry side. Carved out of the paid amount
    -- in both modes: a fully covered registration carries no tax at all.
    IF v_credit > 0 AND v_s.entry_tax_mode <> 'none' THEN
        v_entry_tax := public.compute_entry_tax_cents(
            GREATEST(v_entry_chg - v_credit, 0), 'included');
    END IF;
