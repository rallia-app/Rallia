-- Migration: Add p_has_open_slots filter to search_facilities_nearby[_count]
-- Description: Adds a stricter availability filter on top of the existing
--              p_has_availabilities (bookable) check. While p_has_availabilities
--              only confirms a facility *can* be booked (provider integration,
--              court_slot, or future one-time availability rows), the new
--              p_has_open_slots filter requires a row in
--              public.facility_availability_snapshot with is_available = TRUE
--              and slot_start in the future — i.e. an actual open court right
--              now. Uses the existing partial index idx_fas_facility_window.
...
COMMENT ON FUNCTION search_facilities_nearby IS
  ...
  'facility_availability_snapshot with is_available = TRUE and slot_start > now().';
