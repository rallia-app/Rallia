-- ============================================================================
-- Migration: Add player-activity signal to suggestion ranking
-- Created: 2026-05-09
-- Description:
--   Active players are more likely to see and act on a suggestion. Adds a
--   bucketed activity score (0–1) based on `profile.last_active_at`, falling
--   back to `profile.created_at` for users who haven't yet generated a session
--   ping. Wired into both suggestion RPCs as a small weight in player_compat
--   (0.10 auth, 0.15 anon) — modest because activity is a proxy, not direct
--   evidence of engagement (responsiveness, when present, is the stronger
--   signal).
--
--   Bucketing on (NOW() − last_active_at):
--     ≤  7 days  →  1.00   (active)
--     ≤ 14 days  →  0.85
--     ≤ 30 days  →  0.70   (engaged this month)
--     ≤ 60 days  →  0.50
--     ≤ 90 days  →  0.30
--     >  90 days →  0.10   (likely churned)
--
--   Auth weight rebalance (sum = 1.0):
--     match_type     0.20 → 0.18
--     skill          0.20 → 0.18
--     duration       0.05 → 0.05  (unchanged)
--     overlap        0.25 → 0.22
--     reputation     0.10 → 0.10  (unchanged)
--     responsiveness 0.20 → 0.17
--     activity            → 0.10  (new)
--
--   Anon: player_compat = 0.60·density + 0.25·responsiveness + 0.15·activity.
-- ============================================================================

-- =============================================================================
-- Helper: per-player activity score
-- =============================================================================

CREATE OR REPLACE FUNCTION public.player_activity_score(p_player_id UUID)
RETURNS NUMERIC
LANGUAGE sql STABLE
...
AS $function$
  SELECT
    CASE
      WHEN COALESCE(p.last_active_at, p.created_at) >= NOW() - INTERVAL '7 days'  THEN 1.00::NUMERIC
      WHEN COALESCE(p.last_active_at, p.created_at) >= NOW() - INTERVAL '14 days' THEN 0.85::NUMERIC
      WHEN COALESCE(p.last_active_at, p.created_at) >= NOW() - INTERVAL '30 days' THEN 0.70::NUMERIC
      WHEN COALESCE(p.last_active_at, p.created_at) >= NOW() - INTERVAL '60 days' THEN 0.50::NUMERIC
      WHEN COALESCE(p.last_active_at, p.created_at) >= NOW() - INTERVAL '90 days' THEN 0.30::NUMERIC
      ELSE 0.10::NUMERIC
    END
...
GRANT EXECUTE ON FUNCTION public.player_activity_score(UUID)
  TO authenticated, anon, service_role;

COMMENT ON FUNCTION public.player_activity_score IS
  'Returns a 0.1–1.0 activity score based on profile.last_active_at (falling '
  'back to profile.created_at). 7d → 1.0, 30d → 0.7, 90d → 0.3, beyond → 0.1.';
...
BEGIN
  ...
  ranked AS MATERIALIZED (
    SELECT
      ...
      -- player_compat blends availability density (0.60), responsiveness (0.25),
      -- and activity (0.15). Sum = 1.0.
      (
...
BEGIN
  ...
  ranked AS MATERIALIZED (
    SELECT
      m.*,
      -- Rebalanced weights: activity adds a 0.10 term. match_type/skill -0.02,
      -- overlap -0.03, responsiveness -0.03. Sum = 1.0.
      (
...
COMMENT ON FUNCTION public.get_match_suggestions_anon(uuid, double precision, double precision, integer, integer) IS
  'Anon-mode match suggestions. player_compat = 0.60·availability_density + '
  '0.25·responsiveness + 0.15·activity. matchup_score = 0.70·player_compat + '
  '0.30·facility_score.';

COMMENT ON FUNCTION public.get_match_suggestions_scored(uuid, uuid, integer, double precision, double precision) IS
  'Auth-mode match suggestions. player_compat = 0.18·match_type + 0.18·skill '
  '+ 0.05·duration + 0.22·overlap + 0.10·reputation + 0.17·responsiveness + '
  '0.10·activity. Symmetric ±0.5 rating filter applied at the opponent-'
  'eligibility stage.';
