-- ============================================================================
-- Migration: Fix timezone-aware date/time filtering in match search RPCs
-- Created: 2026-03-06
-- Description: Fixes a bug where matches were incorrectly filtered out when
--   the server's UTC date was ahead of the match's local timezone date.
--
--   Example: A match at 00:00 America/Toronto on 2026-03-06 would be filtered
--   out at 23:50 Toronto time because CURRENT_DATE (UTC) was already 2026-03-06
--   but the TIME comparison (00:00 > 23:50) failed.
--
--   Fix: Compare the match's date+time against the current date+time in the
--   match's timezone, using a proper timestamp comparison instead of separate
--   date and time comparisons.
-- ============================================================================

-- ===========================================
-- Fix search_matches_nearby
-- ===========================================
...
BEGIN
  ...
  WITH match_distances AS (
    ...
    WHERE
      ...
      -- Combine match_date + start_time into a timestamp interpreted in the match's timezone,
      -- then compare against the current moment.
...
-- ===========================================
-- Fix search_public_matches (with p_match_tier)
-- ===========================================

-- First, drop the OLD overload without p_match_tier to resolve ambiguity
DROP FUNCTION IF EXISTS search_public_matches(
  DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, UUID,
  TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT,
  DATE, INT, INT, TEXT, UUID
);
