-- ============================================================================
-- Leagues — league_update: the rules merge, ON TOP of the bounded drain
-- ============================================================================
-- Fixes a regression introduced two migrations ago. 20260807280000 rebuilt
-- league_update to merge and validate default_rules, but it was copied from
-- 20260716180000 rather than from the function's LATEST definition, which was
-- 20260730160100. That silently reverted the bounded waitlist drain: raising a
-- league's capacity stopped promoting queued players into the new seats.
--
-- Caught by league_fairness_and_capacity_test, which asserts exactly that
-- ("raising the cap must promote the whole queue into the new seats").
--
-- This definition is 20260730160100's body — drain included, still bounded by
-- the seats actually opened — with the rules merge and validation applied on
-- top. 20260807280000 is left alone: it has been applied, and forward is the
-- only safe direction.
-- ============================================================================
...
replaced. Raising member_capacity drains the waitlist into the seats that
