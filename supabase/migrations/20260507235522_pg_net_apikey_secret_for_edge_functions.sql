-- Aligns every pg_net caller (cron jobs + trigger/RPC functions) with the
-- canonical Supabase Edge Function auth pattern documented at:
--   https://supabase.com/docs/guides/functions/auth
--   https://supabase.com/docs/guides/getting-started/api-keys
--
-- Sends the project's secret API key (`sb_secret_...`) in the `apikey:` header
-- (NOT `Authorization: Bearer`, which is reserved for user JWTs). The function
-- side validates this via `withSupabase({ auth: 'secret:default' })` from
-- `@supabase/server`, which fails closed if the header is missing or wrong.
--
-- Vault entry naming: keep `service_role_key` — it already holds the new
-- `sb_secret_...` value.
...
-- 2. Trigger / RPC functions — same swap to apikey + service_role_key
...
BEGIN
  ...
  SELECT decrypted_secret INTO functions_url FROM vault.decrypted_secrets WHERE name = 'supabase_functions_url' LIMIT 1;
  ...
  IF functions_url IS NULL OR service_key IS NULL THEN
    RAISE WARNING 'Cannot send invitation email: vault secrets missing';
    RETURN NEW;
  END IF;
...
BEGIN
  ...
  IF functions_url IS NULL OR service_key IS NULL THEN
    RAISE WARNING 'Cannot dispatch notification: vault secrets missing';
    RETURN NEW;
  END IF;
...
BEGIN
  ...
  IF functions_url IS NULL OR service_key IS NULL THEN
    RAISE WARNING '[notify_admin_new_feedback] vault secrets missing';
    RETURN NEW;
  END IF;
  ...
  payload := jsonb_build_object(
    'feedback_id', NEW.id, 'category', NEW.category, 'module', NEW.module,
    'subject', NEW.subject, 'message', NEW.message, 'metadata', NEW.metadata,
    'player_id', NEW.player_id,
    'player_name', COALESCE(player_name, NULL),
    'player_email', COALESCE(player_email, NULL),
    'app_version', NEW.app_version, 'device_info', NEW.device_info,
    'screenshot_urls', NEW.screenshot_urls, 'created_at', NEW.created_at
  );
...
BEGIN
  ...
  IF functions_url IS NULL OR service_key IS NULL THEN
    RAISE WARNING 'Cannot send match interest email: vault secrets missing';
    RETURN NEW;
  END IF;
  ...
  BEGIN
    SELECT INTO request_id net.http_post(
      url := functions_url || '/functions/v1/send-email',
      body := jsonb_build_object(
        'emailType', 'match_interest', 'id', NEW.id, 'match_id', NEW.match_id,
        'email', NEW.email, 'name', NEW.name, 'locale', NEW.locale, 'created_at', NEW.created_at
      ),
      headers := jsonb_build_object('Content-Type', 'application/json', 'apikey', service_key),
      timeout_milliseconds := 10000
    );
...
DECLARE
  ...
  error_msg TEXT;
BEGIN
  IF NOT (NEW.onboarding_completed = TRUE AND (OLD.onboarding_completed IS DISTINCT FROM TRUE)) THEN
    RETURN NEW;
  END IF;
  IF NEW.email IS NULL THEN RETURN NEW; END IF;
  ...
  IF functions_url IS NULL OR service_key IS NULL THEN
    RAISE WARNING 'Cannot send welcome email: vault secrets missing';
    RETURN NEW;
  END IF;
  ...
  BEGIN
    SELECT INTO request_id net.http_post(
      url := functions_url || '/functions/v1/send-email',
      body := jsonb_build_object(
        'emailType', 'welcome', 'user_id', NEW.id, 'email', NEW.email,
        'display_name', NEW.display_name, 'first_name', NEW.first_name,
...
BEGIN
  ...
  IF functions_url IS NULL OR service_key IS NULL THEN
    RAISE WARNING '[notify_admin_push_on_critical_alert] vault secrets missing';
    RETURN NEW;
  END IF;

  v_request_body := jsonb_build_object(
    'alertId', NEW.id, 'alertType', NEW.alert_type, 'title', NEW.title,
    'message', COALESCE(NEW.message, ''), 'severity', NEW.severity,
    'data', jsonb_build_object('action_url', NEW.action_url, 'created_at', NEW.created_at)
  );
...
CREATE OR REPLACE FUNCTION public.send_admin_broadcast_push(
  p_title text, p_message text, p_severity text DEFAULT 'info'::text,
  p_alert_type text DEFAULT 'broadcast'::text, p_admin_ids uuid[] DEFAULT NULL::uuid[]
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  v_request_body := jsonb_build_object(
    'alertType', p_alert_type, 'title', p_title, 'message', p_message, 'severity', p_severity
  );
