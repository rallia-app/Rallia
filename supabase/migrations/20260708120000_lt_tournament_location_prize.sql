-- ============================================
-- Leagues & Tournaments — tournament location (facility OR city) + prize money
-- ============================================
-- Two additions to tournaments:
--   * city            — set either from the picked facility (denormalized) or a
--                       city-only entry when the organizer doesn't have an exact
--                       venue. Card/detail labels fall back to it.
--   * prize_money_cents — optional advertised prize, in the tournament currency.
--
-- Threads both through the create & update RPCs. tournament_create also starts
-- persisting venue_address (previously dropped on the floor) so a picked
-- facility's address is retained.
-- ============================================

ALTER TABLE tournaments
    ADD COLUMN city             text,
    ADD COLUMN prize_money_cents integer
        CHECK (prize_money_cents IS NULL OR prize_money_cents >= 0);


-- ---------------------------------------------------------------- create
-- Adding params changes the signature, so drop the current 20-arg overload
-- (…numeric, jsonb) before recreating with venue_address / city /
-- prize_money_cents appended; re-grant the new signature after.
...
-- ---------------------------------------------------------------- update
-- Same body as 20260629120000 plus `city` (venue window) and
-- `prize_money_cents` (editable while the event still runs — it's advertising,
-- not a payment obligation) in the per-field gate and the UPDATE set.
