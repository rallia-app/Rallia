-- Apply the "≥ 5 events to publish a tier" threshold inside the suggestion
-- RPCs. Without this, `player_reputation.reputation_tier` (e.g. 'silver') was
-- returned verbatim even when the player had < 5 reputation events, while
-- the PlayerProfile screen recomputes the tier client-side via
-- `getTierForScore` and falls back to 'unknown' under the same condition.
-- This caused the SuggestionCard to disagree with the profile (silver vs
-- "new player").
--
-- Both `get_match_suggestions_anon` and `get_match_suggestions_scored` now
-- read `total_events` and gate the returned tier on the same threshold the
-- client uses (MIN_EVENTS_FOR_PUBLIC = 5). Everything else is identical to
-- the previous definitions in 20260509150000_suggestion_responsiveness_signal.
...
BEGIN
  ...
  SELECT
    ...
    -- Hide the stored tier until the player has earned enough reputation
    -- events for it to be considered established (matches client logic in
    -- `getTierForScore`).
