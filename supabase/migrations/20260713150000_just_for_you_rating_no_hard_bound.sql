-- ----------------------------------------------------------------------------
-- get_just_for_you — drop the min-rating hard filter, rely on weighted scoring
--
-- 20260713120000 bounded the match pool to [caller, caller + 0.5] on a match's
-- minimum rating. A hard bound leaves the "Just for you" carousel empty when no
-- in-band game exists nearby. Remove the rating filter entirely: rating
-- proximity is already a weighted signal in the score (score_skill → 0.18 of
-- player_compat, which is 0.55 of the final score and 0.70 of pool ranking), so
-- appropriately-rated games rank at the top while off-rating games still fill
-- the section instead of blanking it.
--
-- Only the candidate-match WHERE clause (and its now-unused rating_score join)
-- change; the rest of the body is a verbatim reproduction of the applied
-- definition.
-- ----------------------------------------------------------------------------
