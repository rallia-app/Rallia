-- =============================================================================
-- Nearby-match notification: gate on the player's ACTIVE rating, exact match
--
-- Previously a player was notified about a public match when they held ANY
-- rating for the sport whose value fell within ±0.5 of the match minimum. That
-- (a) used a fuzzy band and (b) looked at every rating the player had, not the
-- one they actually represent.
--
-- New rule: when the match sets a minimum, notify only players whose ACTIVE
-- rating for the sport (player_sport.active_rating_score_id, added in
-- 20260608140000) is the EXACT SAME rating as the match minimum — same
-- rating_score, i.e. same system AND same value. Matches set their minimum in
-- the sport's primary system (tennis→NTRP, pickleball→DUPR); a player whose
-- active rating is in a different system (self / UTR) therefore won't match,
-- which is intended. Players with no active rating for the sport are excluded.
--
-- Only the rating gate inside nearby_players changes; everything else (location
-- resolution, group/participant exclusion, sport-active + gender filters, the
-- notification payload) is unchanged. v_min_rating_score is still resolved for
-- the human-readable label in the body.
-- =============================================================================
...
BEGIN
  ...
  nearby_players AS (
    ...
    WHERE p.id != NEW.created_by
      ...
      -- Rating gate: when the match sets a minimum, the player's ACTIVE rating
      -- for this sport must be the exact same rating as the match minimum
      -- (same rating_score → same system + value). Players with no active
      -- rating for the sport are excluded.
      AND (
        ...
        OR EXISTS (
          ...
          WHERE ps_rating.player_id = p.id
            ...
            AND prs.rating_score_id = NEW.min_rating_score_id
        )
      )
      AND (
