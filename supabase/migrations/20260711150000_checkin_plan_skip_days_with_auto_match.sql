-- =============================================================================
-- Weekly check-in planner: skip (sport, day) slots that already have a live
-- auto match — not just the exact hour
--
-- Companion to 20260711130000 (one proposal per sport per day). That fixed
-- duplicates WITHIN one preview, but the planner's idempotency guard was still
-- per-hour, so a RE-ENTERED wizard (e.g. second check-in of the window) could
-- propose tennis Sun 17:00 while a tennis Sun 20:00 auto match already exists.
-- record_weekly_checkin enforces one auto match per (sport, date), so that
-- card could never be created — approved and silently skipped.
--
-- The guard now matches the executor's rule exactly: any live auto match for
-- the (sport, date) removes that whole day from the sport's candidate slots.
-- =============================================================================
...
COMMENT ON FUNCTION public.plan_weekly_matches_for_player(uuid, jsonb, int) IS
  ...
  'for the sport — mirroring record_weekly_checkin''s one-auto-match-per-'
  '(sport, date) rule — and returns the top (weekly goal − committed) settings.';
