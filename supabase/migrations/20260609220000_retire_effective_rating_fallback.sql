-- =============================================================================
-- Retire the effective-rating fallback: active rating is the only read path
--
-- 20260608140000 introduced player_sport.active_rating_score_id, backfilled it,
-- and 20260608170000 made the suggestion/search RPCs PREFER it while keeping
-- the certified-then-most-recent heuristic as fallback. Coverage is complete
-- (every rated player_sport has an active rating, verified in prod), so this
-- migration removes the fallback entirely:
--
--   * Six RPCs (get_just_for_you, get_match_suggestions_anon,
--     get_match_suggestions_scored, get_nearby_public_matches,
--     get_upcoming_matches_scored, search_players_nearby) — per-candidate
--     effective_rating CTEs and caller-rating lookups become a direct
--     player_sport.active_rating_score_id join. This also deletes the
--     per-candidate DISTINCT ON scans over player_rating_score (perf lever for
--     the slow suggestion RPCs). get_just_for_you's has_certified_joined now
--     reads the participant's ACTIVE rating's certification, consistent with
--     "you represent your active rating".
--
--   * Two lifecycle triggers keep coverage complete so NULL never regresses:
--       - promote-on-delete: deleting the active rating's row promotes the
--         next-best remaining rating (certified-then-recent — the heuristic's
--         only remaining home, as a default the player can change);
--       - adopt-on-insert: a (re)created player_sport row adopts the player's
--         best existing rating for the sport (auto-activate only fires on
--         rating INSERT, so this closes the re-added-sport gap).
--
--   * Idempotent backfill repairs any row that slipped through (1 in prod).
--
-- The certified-equivalence predicate (badge/referrals/proofs) still appears in
-- certification functions and badge display — that's certification semantics,
-- not rating resolution, and is intentionally untouched.
-- =============================================================================

-- ----------------------------------------------------------------------------
-- Lifecycle trigger 1: promote the next-best rating when the active one is
-- deleted (the FK is ON DELETE SET NULL; read paths no longer fall back).
-- ----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.promote_next_active_rating()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  v_sport_id uuid;
BEGIN
  SELECT rsys.sport_id INTO v_sport_id
    FROM rating_score rs
    JOIN rating_system rsys ON rsys.id = rs.rating_system_id
   WHERE rs.id = OLD.rating_score_id;
  IF v_sport_id IS NULL THEN
    RETURN OLD;
  END IF;

  -- Pointer may already be NULL (FK action ran) or still reference the deleted
  -- row (action pending) depending on trigger ordering — handle both.
  UPDATE player_sport ps
     SET active_rating_score_id = nb.id
    FROM (
      SELECT prs.id
        FROM player_rating_score prs
        JOIN rating_score rs   ON rs.id   = prs.rating_score_id
        JOIN rating_system rsys ON rsys.id = rs.rating_system_id
       WHERE prs.player_id = OLD.player_id
         AND rsys.sport_id = v_sport_id
         AND prs.id <> OLD.id
       ORDER BY
         CASE
   ...
   WHERE ps.player_id = OLD.player_id
     AND ps.sport_id  = v_sport_id
     AND (ps.active_rating_score_id IS NULL OR ps.active_rating_score_id = OLD.id);

  RETURN OLD;
END;
$$;

DROP TRIGGER IF EXISTS player_rating_score_promote_active ON public.player_rating_score;
CREATE TRIGGER player_rating_score_promote_active
  AFTER DELETE ON public.player_rating_score
  FOR EACH ROW EXECUTE FUNCTION public.promote_next_active_rating();

-- ----------------------------------------------------------------------------
-- Lifecycle trigger 2: a new player_sport row adopts the player's best existing
-- rating for the sport when none is set. Runs before the validator (alphabetical
-- BEFORE-trigger order: adopt < validate).
-- ----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.adopt_active_rating_on_player_sport()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp
...
BEGIN
  IF NEW.active_rating_score_id IS NULL THEN
    SELECT prs.id INTO NEW.active_rating_score_id
      FROM player_rating_score prs
      JOIN rating_score rs   ON rs.id   = prs.rating_score_id
      JOIN rating_system rsys ON rsys.id = rs.rating_system_id
     WHERE prs.player_id = NEW.player_id
       AND rsys.sport_id = NEW.sport_id
     ORDER BY
       CASE
         WHEN prs.badge_status = 'certified'::badge_status_enum
           OR prs.is_certified OR prs.referrals_count >= 3 OR prs.approved_proofs_count >= 1 THEN 2
...
DROP TRIGGER IF EXISTS player_sport_adopt_active_rating ON public.player_sport;
CREATE TRIGGER player_sport_adopt_active_rating
  BEFORE INSERT ON public.player_sport
  FOR EACH ROW EXECUTE FUNCTION public.adopt_active_rating_on_player_sport();

-- ----------------------------------------------------------------------------
-- Repair backfill: any rated player_sport still missing an active rating
-- (idempotent; 1 known row in prod from a pre-trigger active-rating delete).
-- ----------------------------------------------------------------------------
WITH best AS (
  SELECT DISTINCT ON (prs.player_id, rsys.sport_id)
    prs.player_id, rsys.sport_id, prs.id AS player_rating_score_id
  FROM player_rating_score prs
  JOIN rating_score rs   ON rs.id   = prs.rating_score_id
  JOIN rating_system rsys ON rsys.id = rs.rating_system_id
...
SET active_rating_score_id = b.player_rating_score_id
FROM best b
WHERE b.player_id = ps.player_id
  AND b.sport_id  = ps.sport_id
  AND ps.active_rating_score_id IS NULL;

-- ----------------------------------------------------------------------------
-- The six RPCs, fallback removed (bodies generated from the live definitions).
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- get_network_pulse: member skill display reads the active rating too (was
-- most-recent-assigned, the last remaining non-active resolution read path).
-- ----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.get_network_pulse(p_network_id uuid, p_days_back integer DEFAULT 30)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  player_skill AS (
    SELECT
      ...
      prs.is_certified
    FROM public.player_sport eps
    JOIN public.player_rating_score prs ON prs.id = eps.active_rating_score_id
    JOIN public.rating_score  rsc ON rsc.id = prs.rating_score_id
    JOIN public.rating_system rsy ON rsy.id = rsc.rating_system_id
    WHERE eps.sport_id = v_network_sport_id
      AND eps.player_id IN (SELECT player_id FROM members)
  ),
