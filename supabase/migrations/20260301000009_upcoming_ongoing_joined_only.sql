-- Migration: Only show ongoing matches in upcoming tab if user has joined status
-- Description: Changes the upcoming filter so that once a match's start_time has passed,
--              it only stays in the upcoming tab if the user has 'joined' status or is the
--              match creator. Previously, ongoing matches were shown if the match was full.
--              This prevents waitlisted/pending/requested users from seeing ongoing matches
--              they aren't confirmed for.
      ...
      CASE
        WHEN p_time_filter = 'upcoming' THEN
          ...
          AND (
            ...
            END)
            OR
            -- Ongoing (start_time passed): only show if user is confirmed (joined) or is the creator
...
COMMENT ON FUNCTION get_player_matches(UUID, TEXT, UUID, INT, INT, TEXT) IS 'Get matches for a player with optional status filtering. Upcoming shows matches where start_time has not been reached (any participant status) or ongoing matches where user has joined status or is creator. Past includes expired matches as soon as start_time passes. Supports filters: all, hosting, confirmed, waiting, needs_players, private (upcoming), feedback_needed, completed, hosted, unfilled, cancelled, private (past). Legacy filters pending, requested, waitlisted, played, expired still work.';
