-- ============================================
-- Leagues & Tournaments — V1: tournament_create RPC
-- ============================================
-- Spec: specs/17-leagues-tournaments/rollout.md §V1 — Tournament create
--       specs/17-leagues-tournaments/data-model.md (tournaments table + RPC table)
--       specs/17-leagues-tournaments/permissions.md §Sport-scope enforcement
--                                                  §Anti-abuse (rate limit)
--
-- Creates a draft tournament. Validates:
--   - Caller plays the requested sport (player_sport, is_active)
--   - Optional network_id references a network of type 'community'
--   - Soft rate limit: max 5 tournaments / organizer / 24h
--
-- Writes an audit row in the same transaction.
--
-- All other tournament fields default per the table definition; this RPC
-- intentionally only takes the minimum-required fields plus the most
-- common optionals. Less-common fields (gates, surfaces, custom seeds)
-- can be set via tournament_update later.
-- ============================================
...
COMMENT ON FUNCTION public.tournament_create(
    text, uuid, smallint, timestamptz, timestamptz,
    text, tournament_visibility, tournament_registration_mode,
    bracket_type, match_format, entry_format,
    uuid, text, uuid, timestamptz, timestamptz
) IS 'Creates a draft tournament. SECURITY DEFINER: bypasses RLS to write the audit row. Spec: specs/17-leagues-tournaments/data-model.md §RPC surface.';
