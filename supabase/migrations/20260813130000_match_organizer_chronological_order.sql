-- ============================================================================
-- Match Organizer: order the card chronologically
-- ============================================================================
-- The snapshot was ordered `court_confirmed DESC, slot_start`, so a bookable
-- court on the 18th was listed above free hours on the 14th. Regeneration then
-- appended pinned options (stale ones, and participant-proposed custom slots)
-- after everything else, so a proposal for tomorrow rendered below engine slots
-- two weeks out.
--
-- Players read this list like a calendar. It is now purely chronological in both
-- serializers, and pinned options merge in by date instead of trailing. The
-- bookable slot still stands out through its green court chip and price.
--
-- Votes are stored positionally in match_time_vote.option_index. The poster
-- writes a card that has no votes yet, and the regenerator re-anchors votes by
-- option_key after the final order is decided, so neither path can re-point a
-- vote at a different slot. match_organizer_add_custom_option still APPENDS for
-- exactly that reason: inserting mid-array would silently move existing votes.
-- The client sorts for display and carries each option's true index along, which
-- is what puts an appended proposal in its chronological place on screen and
-- also fixes cards snapshotted before this migration.
-- ============================================================================
