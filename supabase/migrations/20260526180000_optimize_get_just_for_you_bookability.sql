...
BEGIN
  ...
  caller_opp_overlap AS (
    SELECT oa.player_id AS opp_id, ca.day, ca.hour_of_day
     ...
     WHERE oa.player_id IN (SELECT opp_id FROM shared_fac_opponents)
  ),
  overlap_counts AS (
...
COMMENT ON FUNCTION public.get_just_for_you(
  ...
  'Single round-trip composer for the Home "Just for you" carousel. Mirrors composeJustForYou (matchScoring + suggestionService + cross-pool dedup) in SQL. Suggestion pool now requires the candidate facility to be favorited by BOTH the caller and the opponent for this sport. Auth-only. Suggestion-pool pre-filters opponents to the shared-favorite-facility set, computes caller-opponent availability overlap once (migration 20260526170000), and computes per-facility bookability once in a MATERIALIZED CTE (migration 20260526180000).';
