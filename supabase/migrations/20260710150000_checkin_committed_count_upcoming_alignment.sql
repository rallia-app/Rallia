-- =============================================================================
-- Weekly check-in: align committed_count with upcoming matches
--
-- committed_count (plan step "games you've already got scheduled") was counting
-- every joined/requested/waitlisted match whose match_date falls in today…today+3,
-- including expired games (start passed, not full) that get_player_matches hides
-- from My Matches → upcoming. That inflated the wizard's planned/committed total.
--
-- Fix: centralize the count in count_checkin_window_committed_matches(), mirroring
-- get_player_matches upcoming semantics (not ended, not expired).
-- =============================================================================

CREATE OR REPLACE FUNCTION public.count_checkin_window_committed_matches(
  p_player_id uuid,
  p_today     date
)
RETURNS int
...
COMMENT ON FUNCTION public.count_checkin_window_committed_matches(uuid, date) IS
  'Games already lined up in the check-in rolling window (today…today+3) that '
  'count toward the weekly goal cap. Mirrors get_player_matches upcoming: '
  'joined/requested/waitlisted, not cancelled, no result, not ended, not expired.';

REVOKE ALL ON FUNCTION public.count_checkin_window_committed_matches(uuid, date) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.count_checkin_window_committed_matches(uuid, date) TO service_role;


-- -----------------------------------------------------------------------------
-- plan_weekly_matches_for_player — use shared committed counter
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- get_checkin_match_plan — use shared committed counter
-- -----------------------------------------------------------------------------
