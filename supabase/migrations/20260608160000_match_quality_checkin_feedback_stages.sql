-- Migration: extend get_match_quality_analytics with check-in & feedback funnel stages
-- Adds two intermediate funnel stages between "filled" and "played", measured over
-- filled matches:
--   * all_checked_in : every joined participant has checked_in_at set
--   * all_feedback   : every joined participant has feedback_completed = true
-- These power the auto-generated match funnel in the admin Matches tab:
--   created -> filled -> all checked in -> all gave feedback -> quality.
-- Both are participant-confirmation milestones (distinct from showed_up, which is the
-- post-match attendance flag the strict quality gates rely on).

-- Return type changes (new columns), so the existing function must be dropped first.
...
COMMENT ON FUNCTION get_match_quality_analytics IS
  'Daily, per-sport match lifecycle funnel keyed on play date (match_date): created -> filled -> all checked in -> all gave feedback -> played (>=2 present) -> quality (strict). Check-in/feedback milestones are measured over filled matches from match_participant.checked_in_at / feedback_completed. Strict quality requires played + full attendance + punctual + avg rating >= 4 + no reports, judged from closure aggregates. Returns drop-off branches, quality-miss diagnostics, rating sum/count, and feedback coverage. Rows split by is_auto_generated.';
