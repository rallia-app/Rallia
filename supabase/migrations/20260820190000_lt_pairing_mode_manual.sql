-- ============================================================================
-- Leagues — what 'manual' pairing means to the generator
-- ============================================================================
-- Companion to 20260820180000, which added the enum value.
--
-- Manual does not mean "no sheet": the rounds, the byes and the rotation are
-- structure, not opinion, and an organizer building a night from a blank page
-- would be re-deriving them by hand. It means the system stops CHOOSING who
-- meets whom. The roster is laid out in the order the players confirmed, and
-- the organizer rearranges it with session_swap_player before publishing.
--
-- That is enough to reach any arrangement: a swap inside a round transposes
-- two players, and transpositions generate every permutation. Combined with
-- the draft sheet from 20260820170000, the organizer arranges the whole night
-- before a single member sees it.
--
-- Same body as 20260820170000 otherwise.
-- ============================================================================
...
BEGIN
    ...
    CASE v_session.pairing_mode
        ...
        WHEN 'manual' THEN
            -- The system holds no opinion: the roster keeps the order the
            -- players confirmed in, which the organizer watched happen and can
            -- predict. Anything cleverer is something they would have to undo
            -- pairing by pairing. Filtering on v_sorted keeps the locked
            -- players out, exactly as the ranking query already did.
            SELECT coalesce(
                     array_agg(sp.user_id ORDER BY sp.responded_at NULLS LAST, sp.user_id),
                     ARRAY[]::uuid[])
              INTO v_sorted
              FROM session_presence sp
             WHERE sp.session_id = p_session_id
               AND sp.status     = 'confirmed'
               AND sp.user_id = ANY (v_sorted);
        ELSE
...
roster (random shuffles, avoid_repeat walks least-met, manual keeps
confirmation order and leaves the choosing to the organizer, the rest keep
ranking order); byes, locked rows and round rotation consume that order
unchanged. Leaves the sheet in DRAFT (sheet_published_at = NULL):
session_publish_sheet releases it to the members.';
