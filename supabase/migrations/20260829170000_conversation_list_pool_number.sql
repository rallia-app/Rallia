-- =============================================================================
-- The inbox names pool rooms: tournament_pool_number joins the list RPCs.
-- =============================================================================
-- Since 20260826230000 a tournament can hold one event room plus one room per
-- pool, all conversation_type = 'tournament' with title = the tournament name.
-- The list RPCs never returned the pool number, so the inbox shows identical
-- rows with nothing to tell them apart. The column rides along and the client
-- renders "{name} - Poule {n}" per locale; the shared title stays one string
-- for every reader, exactly as the room's own header already does.
--
-- Adding an OUT column changes the return type, so both functions are dropped
-- and recreated. Bodies are copied from 20260821180000 (verified byte-identical
-- against the live local definitions before editing); the only edits are the
-- appended column in RETURNS and the select lists. Grants re-issued verbatim.
-- =============================================================================

DROP FUNCTION IF EXISTS public.get_player_conversations_optimized(uuid, uuid);
DROP FUNCTION IF EXISTS public.get_player_conversations_filtered(uuid, text, text, int, int, uuid);
...
RETURNS TABLE (
  ...
  tournament_pool_number smallint
)
LANGUAGE plpgsql
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.get_player_conversations_optimized(uuid, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_player_conversations_optimized(uuid, uuid) TO authenticated;

REVOKE ALL ON FUNCTION public.get_player_conversations_filtered(uuid, text, text, int, int, uuid) FROM PUBLIC, anon;
