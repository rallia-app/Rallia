-- =============================================================================
-- Weekly check-in planner: at most ONE proposal per (sport, day)
--
-- The planner deduped candidates per (sport, date, START_TIME), so it could
-- propose the same sport twice on the same day at different hours (e.g. tennis
-- Sat 09:00 AND Sat 10:00). record_weekly_checkin's executor enforces one auto
-- match per (sport, date) — a match created earlier in the SAME submit loop
-- counts — so confirming both cards on the wizard's plan deck created the
-- first game and silently skipped the second (WARNING + CONTINUE), leaving the
-- All-Set recap short of what the player approved.
--
-- Fix at the source: after picking the best facility per slot, keep only the
-- best slot per (sport, date) — ranked by compatible players, then court
-- availability, then the earlier hour. Every card the deck offers can now
-- actually be created, and suggestions spread across days instead of stacking
-- near-duplicates on one afternoon. The executor's (sport, date) guard stays
-- as-is: it still protects the legacy path and stale-plan resubmissions.
-- =============================================================================
...
BEGIN
  ...
  -- One proposal per (sport, day): record_weekly_checkin creates at most one
  -- auto match per sport per date, so offering two hours of the same game
  -- would let the player confirm a card that can never be created.
  best_per_day AS (
...
COMMENT ON FUNCTION public.plan_weekly_matches_for_player(uuid, jsonb, int) IS
  ...
  'slot per (sport, day) — mirroring record_weekly_checkin''s one-auto-match-'
  'per-(sport, date) rule — and returns the top (weekly goal − committed) settings.';
