-- ============================================================================
-- Circuit Rallia — earned points snap to the dime
-- ============================================================================
-- 20260717140000 snapped the combined multiplier to 0.2 steps, which makes the
-- advertised champion number a clean multiple of 100 — but the other placements
-- multiply bases 180/90/20 (not multiples of 50) by that multiplier and still
-- land on residue: a x2.6 draw paid 468 / 234 / 52. Players see those numbers
-- on their ledger and the board.
--
-- Fix in the award itself: each placement's points round to the nearest 10
-- (ATP tables are dime-grained too: 720/360/180/45). Neutral at x1.0 — the
-- base scale 500/300/180/90/20 is already dime-aligned, so no-floor draws pay
-- exactly what they did before, and the existing award tests hold unchanged.
-- Base weights are untouched (the balanced-weighting decision stands); only
-- the final rounding grain changes. History is not re-awarded: recompute is
-- keep-higher on conflict, and re-pricing past results is a product decision
-- we haven't made.
--
-- Function body is otherwise identical to 20260716230100.
-- ============================================================================
