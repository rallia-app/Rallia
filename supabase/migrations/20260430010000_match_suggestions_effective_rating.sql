-- Use sport-scoped, certification-ranked effective rating in match suggestions.
-- Previously both get_match_suggestions_anon and get_match_suggestions_scored chained
-- LEFT JOINs through player_rating_score / rating_score / rating_system without
-- DISTINCT ON or any certification ranking. For opponents with multiple
-- player_rating_score rows (different sports, or self_declared + certified), the
-- planner returned an arbitrary row — sometimes from a different sport, sometimes
-- a stale/lower-priority badge — making the suggestion card's rating disagree with
-- the player directory (which uses the effective_rating CTE in search_players_nearby).
--
-- This migration:
--   1. Adds an effective_rating CTE in both RPCs that picks the best row per player:
--        - sport-scoped (rating_system.sport_id = p_sport_id)
--        - DISTINCT ON (player_id)
--        - ranked: certified-equivalent > self_declared > disputed,
--          tiebroken by assigned_at DESC
--   2. Applies the same ranking to the caller-rating lookup in _scored.
...
BEGIN
  ...
  ranked AS MATERIALIZED (
    ...
    FROM matchups m
    ORDER BY (0.70 * 0.5 + 0.30 * LEAST(m.score_facility, 1.0)) DESC,
             m.dist_caller ASC
    LIMIT p_limit
  )
...
BEGIN
  ...
  -- Caller's effective rating: same DISTINCT ON / certification ranking as
  -- search_players_nearby, so caller-vs-opponent skill scoring uses the same
  -- rating row the directory shows.
...
ALTER FUNCTION public.get_match_suggestions_scored(uuid, uuid, integer)
  SET work_mem = '32MB';
