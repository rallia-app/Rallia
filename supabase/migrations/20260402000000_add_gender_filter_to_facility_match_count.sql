-- Migration: Add gender eligibility filter to facility match count
-- The upcoming_match_count in search_facilities_nearby was counting all public
-- matches regardless of gender preference, causing a mismatch with the actual
-- matches shown to the user (which are filtered by gender eligibility).
