-- Migration: Add search_players_nearby RPC
-- Description: Server-side player search for the Community directory.
--              Moves filtering, sorting, pagination, and distance computation
--              off the client so infinite-scroll pages are stable.
--              Mirrors the shape of search_facilities_nearby.
...
COMMENT ON FUNCTION public.search_players_nearby IS
  'Server-side player directory search. Returns a paginated, filtered, sorted slice plus total_count (window). Distance in meters via PostGIS when p_latitude/p_longitude provided.';
