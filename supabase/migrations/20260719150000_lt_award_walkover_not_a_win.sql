-- ============================================================================
-- Circuit Rallia — a walkover is not a win
-- ============================================================================
-- `real_wins` guarded byes (other_bye = false) but never looked at status, so
-- it keyed purely off winner_registration_id. tournament_match_status carries
-- 'walkover', 'retired', 'disputed' and 'cancelled'; nothing writes them today
-- (both writers set 'completed'), so this is latent rather than live. The
-- moment a walkover row appears it would count as a full real win, since both
-- slots are non-bye.
--
-- Standard practice, and what our own league scoring already does
-- (20260618140000: status IN ('completed','retired')):
--
--   bye        → advances, not a win        (already handled by other_bye)
--   walkover   → advances, not a win, not a loss
--   retirement → a real win
--
-- So real_wins now counts only played outcomes. Exits are deliberately left
-- alone: exit_round is about the depth a player reached, and a walkover loser
-- did leave at that round. The zero-win floor still decides whether that depth
-- is worth anything.
--
-- Function body is otherwise identical to 20260717160000.
-- ============================================================================
