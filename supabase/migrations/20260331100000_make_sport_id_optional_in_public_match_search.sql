-- Make p_sport_id optional in search_public_matches so the marketing /games page
-- can fetch all public matches across all sports.

-- First, drop the accidental 20-param overload if it exists
...
-- Now drop the real 23-param version so we can recreate with p_sport_id DEFAULT NULL
DROP FUNCTION IF EXISTS search_public_matches(
  DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, UUID,
  TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT,
...
BEGIN
  ...
  match_counts AS (
    SELECT
      ...
      -- Coveted count: only compute when sport_id is known (needed for certification check)
