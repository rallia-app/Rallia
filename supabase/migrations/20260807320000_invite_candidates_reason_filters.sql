-- ============================================================================
-- Invite candidates: let the host filter by the same reasons the badges show
--
-- The list explains itself with badges ("Joué ensemble", "Libre à l'heure du
-- match", ...) but the host could only read them, not act on them. Selecting a
-- reason now narrows the list to candidates who actually carry it.
--
-- Semantics: every selected reason must hold (AND), because the chips are a
-- narrowing tool. NULL or an empty array means no filtering, so existing
-- callers behave exactly as before.
--
-- The filter runs here rather than on the fetched page so paging and the
-- total_count window describe the filtered set. Filtering client-side would
-- have narrowed only the rows already loaded and reported a count for a
-- different set.
--
-- 'nearby' is included even though it is not a stored flag: the UI shows it as
-- a badge (under 5 km) so it has to be filterable like the others.
--
-- The 3-argument version is dropped rather than left alongside. A new
-- defaulted parameter creates a SECOND overload, and PostgREST then cannot
-- resolve a named-argument call (PGRST203).
--
-- Body copied verbatim from 20260807120000 with only the parameter and the
-- WHERE added.
-- ============================================================================

DROP FUNCTION IF EXISTS public.get_match_invite_candidates(uuid, int, int);

CREATE OR REPLACE FUNCTION public.get_match_invite_candidates(
  p_match_id uuid,
  p_limit int DEFAULT 20,
  p_offset int DEFAULT 0,
  -- Reason keys the caller wants ALL of. NULL or empty means no filtering.
...
BEGIN
  ...
  WHERE p_reasons IS NULL OR cardinality(p_reasons) = 0 OR (
        (NOT ('played_together'   = ANY(p_reasons)) OR f.pair_count >= 1)
    AND (NOT ('available_at_slot' = ANY(p_reasons))
         OR (NOT f.has_conflict AND v_hours_total > 0 AND f.avail_hours = v_hours_total))
    AND (NOT ('responds_fast'     = ANY(p_reasons))
         OR (f.resp_received >= 3 AND f.score_responsiveness >= 0.7))
    AND (NOT ('active_recently'   = ANY(p_reasons)) OR f.score_activity >= 1.0)
    AND (NOT ('same_rating'       = ANY(p_reasons))
         OR (v_gate_rating_score_id IS NOT NULL AND f.rating_score_id = v_gate_rating_score_id))
    AND (NOT ('favorite_facility' = ANY(p_reasons)) OR (f.fav_facility OR f.plays_here >= 2))
    AND (NOT ('nearby'            = ANY(p_reasons))
         OR (f.dist_m IS NOT NULL AND f.dist_m < 5000))
  )
...
REVOKE ALL ON FUNCTION public.get_match_invite_candidates(uuid, int, int, text[]) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_match_invite_candidates(uuid, int, int, text[]) TO authenticated;

COMMENT ON FUNCTION public.get_match_invite_candidates(uuid, int, int, text[]) IS
  'Host-only: invite candidates for a match ranked by JFY-style behavioral compatibility with invite-specific weights, plus reason flags for the UI. p_reasons narrows to candidates carrying ALL the given reasons.';
