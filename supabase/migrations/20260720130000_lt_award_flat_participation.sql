-- ============================================================================
-- Circuit Rallia — participation pays a flat 20, and only if you played
-- ============================================================================
-- Two problems with the participation rung after the category-doubling curve
-- (20260720120000) widened the spread:
--
--   1. It scaled with the multiplier, so entering Avancé and losing round one
--      paid 80 while winning a game in Débutant paid 50. That pays more for
--      losing in a hard field than for winning in an easy one, and nudges
--      boundary players to enter up and lose.
--   2. `entries` was every non-bye slot in the draw, status unseen — a no-show
--      whose matches were all walkovers still banked participation points.
--
-- So the rung changes meaning: it rewards showing up and playing, not losing.
--
--   * Flat 20 in every category, never multiplied. Showing up is the same act
--     everywhere; the reward for a harder field lives in the win rungs. The
--     pure-attendance season ceiling collapses to 8 × 20 = 160 — noise against
--     a 2000-point title.
--   * Awarded only to players with ≥1 played real match (status 'completed'
--     or 'retired', both slots non-bye — the same set real_wins counts,
--     20260719150000). A player whose only appearances are byes, walkovers or
--     never-played matches gets NO ledger row: no points, no events_played.
--
-- Theoretical corollary: a "champion" whose every match was a walkover gets
-- no row either. That is deliberate — you cannot win a circuit event without
-- playing it.
--
-- Function body is otherwise identical to 20260719150000.
-- ============================================================================
...
BEGIN
    ...
    scored AS (
        SELECT
            ...
            -- Participation is FLAT: showing up is the same act in every
            ...
            (CASE WHEN ex.placement = 'participated' THEN 20
                  ELSE (round(
                    CASE ex.placement
