-- A removed season member could walk back in through the free-path doors.
--
-- 20260722100000 closed the paid door (season_begin_paid_enrollment now raises
-- ENROLLMENT_REMOVED on a 'disqualified' row), but the two free-path writers
-- from 20260629160100 predate the status and still re-admit:
--
--   * season_enroll only short-circuits on 'enrolled', so a 'disqualified' row
--     falls through to the reuse UPDATE and flips back to 'enrolled'. On a free
--     season that re-admits the removed member outright. On a paid season it
--     works too during the pre-refund window: tg_season_member_requires_payment
--     resolves "paid" by (season_id, user_id) having a succeeded payment, and
--     the removed member's about-to-be-refunded payment still qualifies until
--     the settle cron runs.
--
--   * session_confirm_presence auto-enrolls on confirm via
--     ON CONFLICT DO UPDATE SET status = 'enrolled' WHERE status <> 'enrolled',
--     which flips a 'disqualified' row just the same.
--
-- Both are re-emitted from their 20260629160100 bodies with the guard added.
-- season_enroll raises ENROLLMENT_REMOVED like the paid path. Confirming
-- presence also raises (rather than silently skipping the enroll): letting the
-- confirm through would seat a removed member in the session — taking capacity
-- and a matchup slot — while keeping them off the roster, standings, and the
-- organizer's view. The tournament free path already refuses re-entry the same
-- way (REGISTRATION_REMOVED, 20260721130200).

-- ----------------------------------------------------------------------------
-- season_enroll: refuse an organizer-removed member
-- ----------------------------------------------------------------------------
...
BEGIN
    ...
    -- path's guard in season_begin_paid_enrollment); without this the reuse
    -- UPDATE below re-admits them.
...
-- ----------------------------------------------------------------------------
-- session_confirm_presence: a removed member cannot confirm their way back in
-- ----------------------------------------------------------------------------
...
BEGIN
    ...
    -- never bumps an already-enrolled row, and re-enrolls a withdrawn one — but
    -- never a disqualified one: an organizer-removed member must not confirm
    -- into a seat while off the roster (guard added 20260722110000). FOR UPDATE
    -- serializes against a concurrent season_remove_member; the extra exclusion
    -- in the upsert's WHERE is belt-and-braces for the insert race.
