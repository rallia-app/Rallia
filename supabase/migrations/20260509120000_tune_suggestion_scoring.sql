-- ============================================================================
-- Migration: Tune suggestion scoring
-- Created: 2026-05-09
-- Description:
--   Three changes to the suggestion ranking RPCs based on product feedback:
--
--   1. ANON: replace the hardcoded `player_compat = 0.5` with the opponent's
--      availability density (active cells / 21). Opponents who are flagged for
--      more (day, period) cells rank higher because they're more bookable. No
--      hard cutoff — narrow-availability opponents still appear, just lower.
--
--   2. AUTH: increase the weight of the shared-availability overlap signal
--      from 0.15 to 0.30 (relative to other player_compat components) and
--      tighten the normalization from `/21` to `LEAST(count/7, 1.0)`. After
--      the change, an opponent who shares 7+ (day, period) cells with the
--      caller gets full overlap credit, and overlap is now tied with
--      match_type / skill as a top-tier compat factor.
--
--      Rebalanced weights (sum to 1.0):
--        match_type  0.30 → 0.25
--        skill       0.30 → 0.25
--        duration    0.05 → 0.05
--        overlap     0.15 → 0.30   (and tighter ramp)
--        reputation  0.20 → 0.15
--
--   3. AUTH: rating filter becomes symmetric ABS(<=0.5) instead of one-way
--      ">= caller-0.5". Callers no longer see opponents 0.5+ above their own
--      level. Unrated callers/opponents still bypass the filter.
-- ============================================================================

-- =============================================================================
-- Drop a vestigial 3-arg overload of get_match_suggestions_scored. The current
-- code path (mobile + edge) uses the 5-arg variant added in the 2026-04-30
-- location-override migration. The old 3-arg signature was never dropped, so
-- both still exist — drop it now to keep the function name unambiguous.
-- =============================================================================
DROP FUNCTION IF EXISTS public.get_match_suggestions_scored(uuid, uuid, integer);
...
BEGIN
  ...
  opponents AS (
    SELECT
      ...
      -- New: availability density. count of active (day, period) cells / 21.
      -- Opponents flagged for the whole week → 1.0. One cell → ~0.05.
      (
  ...
  ranked AS MATERIALIZED (
    SELECT
      ...
      -- player_compat now reflects availability density (was hardcoded 0.5).
      m.opp_avail_density                                             AS player_compat,
      (
        0.70 * m.opp_avail_density
...
BEGIN
  ...
  opponents AS (
    SELECT
      ...
      *
      ...
      -- Tightened normalization: 7+ shared cells → 1.0 (was /21, so 21+ → 1.0).
      LEAST(
        (
   ...
   WHERE ps.sport_id    = p_sport_id
     ...
     AND opp.location   IS NOT NULL
     -- Symmetric ±0.5 rating filter (was one-way ">= caller-0.5"). Caller
     -- with no rating, or opponent with no rating, still bypasses.
     AND (
  ...
  ranked AS MATERIALIZED (
    SELECT
      m.*,
      -- Rebalanced weights: overlap doubled (0.15 → 0.30), match_type/skill
      -- each shaved 0.05, reputation shaved 0.05. Sum = 1.0.
      (
        0.25 * m.score_match_type
      + 0.25 * m.score_skill
      + 0.05 * m.score_duration
      + 0.30 * m.score_overlap
      + 0.15 * m.score_reputation
...
COMMENT ON FUNCTION public.get_match_suggestions_anon(uuid, double precision, double precision, integer, integer) IS
  'Anon-mode match suggestions. player_compat reflects opponent availability '
  'density (count of active (day, period) cells / 21). matchup_score = '
  '0.70·player_compat + 0.30·facility_score.';

COMMENT ON FUNCTION public.get_match_suggestions_scored(uuid, uuid, integer, double precision, double precision) IS
  'Auth-mode match suggestions. player_compat = weighted blend of match_type '
  '(0.25), skill (0.25), duration (0.05), shared-availability overlap (0.30, '
  'normalized as LEAST(count/7, 1.0)), reputation (0.15). Symmetric ±0.5 '
  'rating filter applied at the opponent-eligibility stage.';
