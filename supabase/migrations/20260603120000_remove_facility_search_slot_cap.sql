-- Migration: Remove the 60-slot per-facility cap from search_facilities_nearby
-- Description: The inline availability_slots LATERAL previously capped each
--              facility at 60 raw rows (one row = court × timeslot), ordered
--              earliest-first. For multi-court facilities that budget is
--              exhausted within the first day or two of the snapshot window,
--              so later days never reached the client and the mobile
--              FacilityCard appeared to be "missing" availabilities.
--
--              The payload is already bounded by the snapshot itself
--              (sport-scoped, future-only, 7-day window per the refresh
--              worker), so we drop the LIMIT and let every snapshot row for
--              the facility flow through. The inner ordered subquery is no
--              longer needed — jsonb_agg's own ORDER BY preserves slot
--              ordering — so it is flattened to a direct table scan that uses
--              idx_fas_facility_sport_window.
--
--              Signature and return shape are unchanged (CREATE OR REPLACE).
--              search_facilities_nearby_count is unchanged.
...
BEGIN
  ...
  -- (facility_id, sport_id, slot_start) lookup. No per-facility cap: the
  -- snapshot is already sport-scoped, future-only, and bounded to the
  -- refresh worker's 7-day window, so every snapshot row is returned.
  LEFT JOIN LATERAL (
    SELECT jsonb_agg(
      jsonb_build_object(
...
COMMENT ON FUNCTION search_facilities_nearby IS
  ...
  'future-only, all rows within the refresh worker''s 7-day window) so the '
  'mobile list view does not need per-card availability queries. '
  'p_has_availabilities filters to facilities that *could* be booked; '
