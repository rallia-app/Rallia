-- ════════════════════════════════════════════════════════════════════════
-- Optimize get_just_for_you — suggestion-pool hot path
-- ════════════════════════════════════════════════════════════════════════
--
-- Two structural fixes to the suggestion pool (the match pool, scoring math,
-- JSONB payloads, and final merge are UNCHANGED — results are identical):
--
--   #1 Filter-before-score. The `opponents` CTE used to fully score up to 200
--      players for the sport, and only afterwards did `matchups` drop everyone
--      who doesn't share a favorited facility with the caller. We now pre-filter
--      to opponents who share ≥1 favorited facility (`shared_fac_opponents`)
--      BEFORE scoring. Any non-sharing opponent produced zero `matchups` rows
--      anyway, so this is result-equivalent — it just stops scoring rows that
--      were always discarded.
--
--   #2 Compute caller↔opponent availability overlap ONCE. The old code probed
--      `player_availability` per-opponent three times (the EXISTS filter, the
--      `score_overlap` scalar subquery, and the `candidate_slots` join), each a
--      correlated scan with a useless Memoize (0 hits). We now materialize the
--      overlap cells once (`caller_opp_overlap`) and reuse them for the filter,
--      the score, and slot expansion.
--
-- Validated read-only against prod (caller a039e68d / tennis, 175 shared-fac
-- opponents): opponents→matchups stage dropped from ~296ms to ~27ms execution
-- with byte-identical output (same row count + opp/facility checksum).
-- ════════════════════════════════════════════════════════════════════════
...
COMMENT ON FUNCTION public.get_just_for_you(
  ...
  'Single round-trip composer for the Home "Just for you" carousel. Mirrors composeJustForYou (matchScoring + suggestionService + cross-pool dedup) in SQL. Suggestion pool now requires the candidate facility to be favorited by BOTH the caller and the opponent for this sport. Auth-only. Suggestion-pool pre-filters opponents to the shared-favorite-facility set and computes caller-opponent availability overlap once (see migration 20260526170000).';
