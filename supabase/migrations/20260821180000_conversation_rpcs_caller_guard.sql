-- =============================================================================
-- Caller guard for the conversation-list RPCs.
--
-- get_player_conversations_optimized, get_player_conversations_filtered and
-- get_unread_conversations_count are SECURITY DEFINER and take p_player_id
-- with no check against the caller, so any authenticated user could read
-- another player's inbox previews and unread counts. Bodies are copied
-- verbatim from their latest definitions (20260811140000 and 20260312000000)
-- with two additions each: SET search_path = public, and a guard that raises
-- 42501 unless p_player_id is auth.uid(). Every app caller already passes the
-- signed-in user's id; no SQL function calls these internally.
-- =============================================================================
...
REVOKE ALL ON FUNCTION public.get_unread_conversations_count(uuid) FROM PUBLIC, anon;
