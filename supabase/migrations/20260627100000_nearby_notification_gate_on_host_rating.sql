-- =============================================================================
-- Nearby-match notification: gate on the HOST's rating when no minimum is set
--
-- Bug: the rating gate in notify_nearby_players_on_match_created only applied
-- when the match carried an explicit minimum (NEW.min_rating_score_id). When a
-- host created a public match WITHOUT setting a minimum, the gate short-circuited
-- to TRUE and every nearby active-sport player was notified, regardless of how
-- far the host's level was from theirs. e.g. an NTRP 2.5 host's open game pushed
-- to an NTRP 4.5 player. The host's own rating was never consulted.
--
-- Fix: resolve a single gating rating — the explicit match minimum if set,
-- otherwise the HOST's active rating for the sport — and gate recipients on an
-- exact match against it (same rating_score → same system + value), exactly as
-- before. Only when neither a minimum nor a host active rating exists (truly no
-- level info) do we fall back to notifying everyone nearby, preserving reach for
-- unrated hosts.
--
-- Everything else (location resolution, group/participant exclusion, sport-active
-- + gender filters, the payload, the human-readable label which still reflects
-- only an explicit minimum) is unchanged.
-- =============================================================================
