-- ============================================================================
-- Migration: Add match tier filter to search_public_matches
-- Created: 2026-03-01
-- Description: Adds p_match_tier TEXT parameter to search_public_matches RPC.
--   Supports filtering by match tier: 'mostWanted', 'covetedPlayers', 'courtBooked'.
--   Tier logic:
--     - A participant is "coveted" when reputation_score >= 90
--       AND badge_status = 'certified' for the match's sport.
--     - A match has "coveted players" when at least 1 joined player is coveted
--       for singles, or at least 2 for doubles.
--     - mostWanted: court_status = 'reserved' AND has coveted players
--     - covetedPlayers: has coveted players (includes mostWanted)
--     - courtBooked: court_status = 'reserved' (includes mostWanted)
-- ============================================================================
...
CREATE OR REPLACE FUNCTION search_public_matches(
  ...
  p_match_tier TEXT DEFAULT NULL        -- Filter by match tier: 'mostWanted', 'covetedPlayers', 'courtBooked'
)
RETURNS TABLE (
  match_id UUID,
...
BEGIN
  ...
  filtered_matches AS (
    ...
    WHERE
      ...
      -- Each word in the query must match at least one of the searchable fields.
      -- "tennis park" matches rows containing both "tennis" AND "park" anywhere.
      AND (
  ...
  match_counts AS (
    SELECT
      ...
      -- Count coveted participants (reputation >= 90 AND certified badge for this sport)
      (
        SELECT COUNT(*)
        FROM match_participant mp
...
-- Grant permissions (21 params now)
GRANT EXECUTE ON FUNCTION search_public_matches(
  DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, DATE, INT, INT, TEXT, UUID, TEXT
) TO authenticated;
GRANT EXECUTE ON FUNCTION search_public_matches(
  DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, DATE, INT, INT, TEXT, UUID, TEXT
) TO anon;
...
COMMENT ON FUNCTION search_public_matches(
  DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, DATE, INT, INT, TEXT, UUID, TEXT
) IS 'Search public matches with comprehensive filters including match tier (mostWanted/covetedPlayers/courtBooked), word-tokenized accent-insensitive text search, duration, court status, specific date, and facility. Always filters out matches that have already started today. Includes full matches in results.';
