-- =============================================================================
-- Weekly check-in: scope "Games for you" + the match plan to one sport
--
-- The app keeps Tennis and Pickleball as separate universes (sport mode), but
-- the check-in wizard was the outlier: opportunities and plan proposals fanned
-- out across every active player_sport. The wizard now passes the selected
-- sport mode, so both surfaces only show that sport's content.
--
-- p_sport_id is optional on all three functions; NULL keeps the previous
-- all-active-sports behaviour (used while the sport context is still loading,
-- and by any legacy caller). Goal, committed-count and availability stay
-- player-wide — the weekly goal counts games across sports.
--
-- The old signatures are dropped so a 3/4-arg call can't hit an ambiguous
-- overload.
-- =============================================================================

DROP FUNCTION IF EXISTS public.get_checkin_match_opportunities(jsonb, text, int);
DROP FUNCTION IF EXISTS public.get_checkin_match_plan(jsonb, smallint, text, int);
DROP FUNCTION IF EXISTS public.plan_weekly_matches_for_player(uuid, jsonb, int);

-- ---------------------------------------------------------------------------
-- Planner: body identical to 20260711150000 except the sport loop's filter.
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.plan_weekly_matches_for_player(uuid, jsonb, int, uuid) IS
  ...
  'by compatible-player count, keeps the best court per slot, then the best '
  'slot per (sport, day), skipping days that already carry a live auto match '
  'for the sport, and returns the top (weekly goal − committed) settings. '
  'p_sport_id restricts candidates to that sport (NULL = all active sports).';

REVOKE ALL ON FUNCTION public.plan_weekly_matches_for_player(uuid, jsonb, int, uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.plan_weekly_matches_for_player(uuid, jsonb, int, uuid) TO service_role;

-- ---------------------------------------------------------------------------
-- Plan preview: body identical to 20260711140000, sport passed to the planner.
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.get_checkin_match_plan(jsonb, smallint, text, int, uuid) IS
  ...
  'p_sport_id scopes proposals to the wizard''s sport mode (NULL = all active '
  'sports). Invitee candidates are returned regardless of auto_invite_players.';
...
-- ---------------------------------------------------------------------------
-- "Games for you": body identical to 20260619130000 except caller_sports.
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.get_checkin_match_opportunities(jsonb, text, int, uuid) IS
  ...
  'the match''s sport; weekday+hour in a declared slot; gender match. p_sport_id '
  'scopes results to the wizard''s sport mode (NULL = all active sports). '
  'Windowed today…today+3 in player tz. Returns match ids + distance to hydrate.';

REVOKE ALL ON FUNCTION public.get_checkin_match_opportunities(jsonb, text, int, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_checkin_match_opportunities(jsonb, text, int, uuid) TO authenticated, service_role;
