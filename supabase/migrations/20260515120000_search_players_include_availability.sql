-- Migration: Include availability slots in search_players_nearby RPC
-- Description: Returns each player's active availability slots aggregated as
--              a JSONB object keyed by day, with each value being an array of
--              periods. Lets the player directory render an availability grid
--              without an extra round trip.
--
-- Shape of the returned `availability` column:
--   { "monday": ["morning","evening"], "tuesday": ["afternoon"], ... }
-- Days with zero active slots are omitted. NULL when the player has no slots.
...
RETURNS TABLE (
  ...
  -- New column: availability map { day: [periods] }
  availability JSONB
)
LANGUAGE sql
STABLE
...
AS $$
  ...
  availability_agg AS (
    -- Per-player availability map. Inner aggregation produces, for each
    -- (player, day), an ordered array of periods. Outer aggregation rolls
    -- those into a single JSONB object keyed by day name.
    SELECT
      sub.player_id,
      jsonb_object_agg(sub.day_text, sub.periods) AS availability
    FROM (
      SELECT
        pa.player_id,
        pa.day::TEXT AS day_text,
        jsonb_agg(pa.period::TEXT ORDER BY pa.period) AS periods
      FROM public.player_availability pa
      WHERE pa.is_active IS TRUE
      GROUP BY pa.player_id, pa.day
    ) sub
  ...
  filtered AS (
    ...
    LEFT JOIN availability_agg aa ON aa.player_id = p.id
    WHERE
      ...
      AND (
        ...
        OR EXISTS (
          ...
          WHERE pa.player_id = p.id
            AND pa.period::TEXT = p_availability
...
COMMENT ON FUNCTION public.search_players_nearby IS
  'Server-side player directory search. Returns a paginated, filtered, sorted slice with rating, reputation, online status, and availability map. Distance in meters via PostGIS when p_latitude/p_longitude provided.';
