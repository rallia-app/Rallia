-- =============================================================================
-- Auto-match generation: cap to the weekly goal (net of joined games) + direct join
--
-- Two behaviour changes to generate_weekly_matches_for_player, both prompted by
-- the new check-in "Games for you" step (which now lets players JOIN existing
-- public matches before we offer to create new ones):
--
--   1. CAP creation to the player's weekly frequency_goal MINUS the games they
--      already committed to in the window (joined / requested / waitlisted) —
--      including ones they just joined on the "Games for you" step. Previously
--      this created one match per available day per active sport (up to ~8),
--      ignoring the goal — the main driver of the supply glut / low fill rate.
--      Slots are filled SOONEST-FIRST (today…+3, sports interleaved per day,
--      primary sport first).
--
--   2. DIRECT join instead of request-to-join. Auto-created matches were
--      request-mode, so joins died waiting on an absentee auto-host's approval.
--      The check-in's auto-match step now makes the opt-in explicit, so these
--      open games auto-accept joiners.
--
-- generate_weekly_matches_for_all_players delegates here, so the sweep inherits
-- both changes. Everything else (favorite-facility/hour tiering, idempotency per
-- (sport, date), conflict guard, ACL, dispatch) is unchanged.
-- =============================================================================
  ...
  FOR d IN 0..3 LOOP
    ...
    LOOP
      ...
      INSERT INTO public.match (
        ...
        'public', 'direct', 'singles', COALESCE(v_sport.match_type, 'both'), v_duration_enum,
...
COMMENT ON FUNCTION public.generate_weekly_matches_for_player(uuid) IS
  'Creates open (direct-join) matches hosted by the player over the rolling '
  ...
  'the check-in "Games for you" step). Required level = the host''s ACTIVE rating '
  '(player_sport.active_rating_score_id), NULL when absent. Per day/sport it picks '
  'the favorite facility/hour by tier (bookable-with-courts > FCFS > rest, nearest '
  'then earliest), or TBD when no favorites. Gated on auto_create_matches. '
  'Idempotent per (sport, exact date).';
