-- =============================================================================
-- Suggestion scoring: penalize opponents with a `disputed` rating badge
--
-- The existing score_skill formula already applies a badge-confidence
-- multiplier that downweights disputed ratings (caller × opp matrix, e.g.
-- self_declared × disputed = 0.20). But that penalty is *local to the skill
-- component* (weight 0.18 in player_compat) and is bypassed entirely when
-- either side has a NULL rating. Real-world: disputed opponents with strong
-- non-skill signals (availability, responsiveness, history) still surface
-- near the top of the suggestion list.
--
-- Fix: add a flat -0.15 demotion to player_compat for any opponent whose
-- effective badge_status = 'disputed'. Applied inside the existing clamp,
-- so total impact is bounded to [0, 1]. Independent of skill — penalizes
-- disputed status whether or not a rating value is populated.
--
-- The disputed opponent stays in the candidate pool (not excluded) — users
-- can still find them. They just rank well below non-disputed peers with
-- otherwise-comparable signals.
--
-- Scope: auth RPC only (anon path has no badge data — the column would be
-- NULL and the CASE branch is a no-op).
-- =============================================================================
...
COMMENT ON FUNCTION public.get_match_suggestions_scored(uuid, uuid, integer, double precision, double precision) IS
  'Auth match suggestions. player_compat = clamp(<7 content weights sum to 1.0> + 0.5 × score_history − disputed_penalty, 0, 1). '
  ...
  'disputed_penalty = 0.15 when opponent badge_status = ''disputed'' (independent of the skill-component multiplier). '
