 ...
 *
 * --- Redesign: "Editorial white" ---
 * Same API and dark-mode classes; refreshed visuals:
 *   • white header with the colored logo chip (white pill baked into the PNG so
 *     the black letters survive dark mode, incl. Gmail's forced inversion)
 *   • short gold tick under the logo instead of a full-width rule
 *   • dark neutral headings (teal reserved for eyebrows/links)
 *   • boxless hairline detail rows, warm off-white page background
 *   • full-width flat coral CTA, quiet left-aligned footer
  ...
  return `
    <style type="text/css">
      @media (prefers-color-scheme: dark) {
        ...
        .email-header { background-color: ${T.darkContainerBg} !important; }
        .email-content { background-color: ${T.darkContainerBg} !important; }
        .email-footer { background-color: ${T.darkContainerBg} !important; border-top-color: #404040 !important; }
        .email-text { color: ${T.darkText} !important; }
        .email-muted { color: ${T.darkMutedText} !important; }
        .email-eyebrow { color: #5eead4 !important; }
        .email-detail-card td { border-top-color: #3a3a3a !important; border-bottom-color: #3a3a3a !important; }
        .email-game-card { background-color: #2d2d2d !important; border-color: #404040 !important; }
      ...
      [data-ogsc] .email-header { background-color: ${T.darkContainerBg} !important; }
      [data-ogsc] .email-footer { background-color: ${T.darkContainerBg} !important; }
      ...
      [data-ogsc] .email-game-card { background-color: #2d2d2d !important; border-color: #404040 !important; }
                ...
                <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
                  <tr>
                    <td align="left" style="padding: 16px 0 0 0;">
                      <p class="email-muted" style="margin: 0; padding: 0 0 10px 0; font-size: 12px; font-weight: 600; color: ${T.neutral500};">${downloadText}</p>
                      <table role="presentation" cellspacing="0" cellpadding="0" border="0">
                        <tr>
                          <td style="padding: 0 8px 0 0;">
                            <a href="https://apps.apple.com/app/rallia/id6760482014" style="text-decoration: none;">
                              <img src="${siteUrl}/app-store-badge.svg" alt="${appStoreAlt}" width="108" height="36" style="display: block; border: 0;" />
                            </a>
                          </td>
                          <td>
                            <a href="https://play.google.com/store/apps/details?id=com.mathisl971.ralliaapp" style="text-decoration: none;">
                              <img src="${siteUrl}/google-play-badge.svg" alt="${googlePlayAlt}" width="122" height="36" style="display: block; border: 0;" />
                            </a>
                          </td>
                        </tr>
                      </table>
...
export function wrapInLayout(options: LayoutOptions): string {
  ...
  const subtitleHtml = headerSubtitle
    ? `<p class="email-muted" style="margin: 10px 0 0 0; padding: 0; font-size: 13px; font-weight: 600; color: ${T.neutral500}; letter-spacing: 0.02em;">${escapeHtml(headerSubtitle)}</p>`
    : '';
    ...
    <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" class="email-body" style="background-color: ${T.pageBg}; font-family: Inter, Arial, Helvetica, sans-serif;">
      <tr>
        <td align="center" style="padding: 40px 20px;">
          <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="600" class="email-container" style="background-color: ${T.white}; border: 1px solid ${T.hairline}; border-radius: 20px; overflow: hidden;">
            <!-- Header: colored logo chip on white -->
            <tr>
              <td align="left" class="email-header" style="padding: 32px 48px 0 48px; background-color: ${T.white};">
                <img src="${siteUrl}/email-logo-chip.png" alt="Rallia" width="116" height="55" style="display: block; border: 0; max-width: 116px; height: auto; margin-left: -10px;" />
                ${subtitleHtml}
              </td>
            </tr>
            ...
            <!-- Gold tick -->
            <tr>
              <td align="left" class="email-tick" style="padding: 20px 48px 0 48px;">
                <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="44">
                  <tr><td height="3" style="height: 3px; line-height: 3px; font-size: 3px; background-color: ${T.accent500}; border-radius: 2px;">&nbsp;</td></tr>
                </table>
              </td>
            </tr>
            ...
            <!-- Footer -->
            <tr>
              <td align="left" class="email-footer" style="padding: 24px 48px 30px 48px; background-color: ${T.white}; border-top: 1px solid ${T.hairlineSoft};">
                <p class="email-muted" style="margin: 0; padding: 0; font-size: 13px; line-height: 1.6; color: ${T.neutral500};">
                  <span class="email-text" style="font-weight: 600; color: ${T.neutral600};">${needHelpText}</span> ${supportText}
                </p>
                ...
                <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
                  <tr>
                    <td style="padding: 8px 0 28px 0;">
                      <a href="${href}" class="email-cta" style="display: block; text-align: center; background-color: ${T.secondary500}; border-radius: ${T.buttonRadius}; padding: 16px 0; font-family: Inter, Arial, Helvetica, sans-serif; font-size: 16px; font-weight: 700; color: ${T.white}; text-decoration: none; letter-spacing: -0.01em;">
                      ${escapeHtml(text)}
                      </a>
                    </td>
                  </tr>
...
export function renderSecondaryButton(text: string, href: string): string {
  return `
                <table role="presentation" cellspacing="0" cellpadding="0" border="0">
                  <tr>
                    <td>
                      <a href="${href}" style="display: inline-block; padding: 8px 12px; font-family: Inter, Arial, Helvetica, sans-serif; font-size: 14px; font-weight: 600; color: ${T.primary600}; text-decoration: none;">
                        ${escapeHtml(text)}
                      </a>
                    </td>
                  </tr>
...
export function renderDetailCard(rows: Array<{ label: string; value: string }>): string {
  ...
  const rowsHtml = rows
    .map(
      (row, i) => `
                      <tr>
                        <td class="email-detail-label" style="padding: 14px 0; border-top: 1px solid ${T.hairlineSoft}; ${i === lastIndex ? `border-bottom: 1px solid ${T.hairlineSoft};` : ''} color: ${T.neutral500}; font-size: 12px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.08em; width: 110px; vertical-align: middle;">${escapeHtml(row.label)}</td>
                        <td class="email-detail-value" style="padding: 14px 0; border-top: 1px solid ${T.hairlineSoft}; ${i === lastIndex ? `border-bottom: 1px solid ${T.hairlineSoft};` : ''} color: ${T.neutral900}; font-size: 15px; font-weight: 600; vertical-align: middle;">${row.value}</td>
                      </tr>`
    )
    .join('');
                ...
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">
                  <tr>
                    <td style="padding: 4px 0 20px 0;">
                      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" class="email-detail-card">
                        ${rowsHtml}
                      </table>
                    </td>
                  </tr>
                ...
                <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
                  <tr>
                    <td align="center" style="padding: 0 0 24px 0;">
                      <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" class="email-link-box" style="background-color: ${T.pageBg}; border: 1px solid ${T.hairline}; border-radius: 14px; overflow: hidden;">
                        <tr>
...
export function renderDividerAndDisclaimer(text: string): string {
  return `
                <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
                  <tr>
                    <td class="email-divider" style="padding: 16px 0; border-top: 1px solid ${T.hairlineSoft};"></td>
                  </tr>
                </table>
