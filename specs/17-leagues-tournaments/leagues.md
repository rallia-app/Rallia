...
### Sheet publication (20260820170000)

Generating a sheet does not reveal it. `sessions.sheet_published_at` is NULL
until the organizer calls `session_publish_sheet`, and `smatches_select` hides
`session_matches` from everyone but the organizer while it is. This is what
lets an organizer adjust pairings (`session_swap_player`) before members see
them. Regenerating drops the sheet back to draft: a regenerated sheet is a
different sheet, and pairings must not shift under a member between refreshes.

The hiding is enforced in RLS, not in a screen, so it also covers
`listMyUnscheduledSessionMatches`: a draft pairing does not push a "game to
organize" card at a player who should not know it yet.

### Fixed and flex sessions (20260820200000)

`leagues.default_rules.sessionScheduling` picks how the league's sessions run,
and is snapshotted into each season like the rest of the rules object:

| Value             | A session is                                                     | `play_window_ends_at` |
| ----------------- | ---------------------------------------------------------------- | --------------------- |
| `fixed` (default) | one evening at `scheduled_at`, lasting `duration_minutes`        | NULL                  |
| `flex`            | a window from `scheduled_at` to `play_window_ends_at`, days long | set                   |

A flex session needs no new lifecycle: a session completes when every one of
its matches carries a score, not on a clock, so the window is a statement of
intent the UI renders and the organizer polices. `scheduled_at` keeps meaning
"when the session opens", which is why the confirm-reminder cron keeps working
untouched.

`session_create_series` takes `p_window_days` instead of an end instant, since
each occurrence needs its own window, capped at the cadence so two windows can
never claim the same days.

**Open:** what happens to a pairing nobody played by the end of the window.
Nothing happens today when a fixed session's evening passes either, so flex
inherits that rather than adding a gap. The eventual answer is drafted in
[formats/ligue-en-boites.md](./formats/ligue-en-boites.md) section 11
(responsibility-based walkover, or cancel) and should land for both kinds of
session at once.
