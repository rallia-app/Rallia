...
| **V3 — Bracket**                           | `tournament_generate_bracket`, `tournament_reset_bracket`, `tournament_swap_players`. Bracket viewer.                                                                                                                                        | `<ts>_lt_tournament_bracket_rpcs.sql`                  | Bracket render + organizer edit          |
| **V4 — Score entry & completion**          | Match bridge (`tournament_matches.match_id`, `lt_create_match_for_tournament_match`, sync trigger → advance winner), `attach_match_to_tournament_slot`, `tournament_override_score`. Reuses `RegisterMatchScoreSheet` + link-match sheet.    | `<ts>_lt_tournament_match_bridge.sql` (+ override)     | Link/play + organizer override           |
...
| **V6 — League create + season open**       | `league_create`, `league_join`, `league_approve_member`, `season_create`, `season_open`. Schema delta: `leagues.min_rating`/`max_rating`/`min_reputation`. League list + season toggle.                                                      | `<ts>_lt_league_create_rpcs.sql`                       | League create + member list              |
...
| **V9 — League scoring + ranking**          | Session match bridge (mirror V4), `session_override_score`, `recalc_season_ranking`, ranking UI. Reuses casual match scoring + optional link-match.                                                                                          | `<ts>_lt_session_match_bridge.sql` (+ ranking RPC)     | Ranking tab + session score entry        |
