...
| Δcontest           | 12 h from declaration on a tournament pairing, 48 h on a casual game       | closes early once the next game is played / the phase consumed           |
| restore window     | RR: until the phase is consumed; KO: until the advanced side's next result | organizer one-tap; after the deadline it re-grants the pairing 72 h      |
...
| poules-puis-éliminatoires § scoring        | A forfeit is a defeat with the format's forfeit score (8-0 / 6-0 6-0), in pools and in the draw alike, and it moves the loser's ratios like any defeat. Shipped 2026-08-29 (20260829160000): standings synthesize every walkover from status, winner and format (legacy W/O rows included), and the three walkover writers stamp the visible score via lt_forfeit_score.                                       |
