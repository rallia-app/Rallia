...
| round-deadlines § Organizer surface        | The deadline editor is the missing piece, not the machinery: `tournament_round_deadlines`, `tournament_set_round_deadlines`, `setTournamentRoundDeadlines` and `useSetTournamentRoundDeadlines` all exist and no screen calls them, which is why Jean reports that an organizer cannot enter a deadline today.                          |
...
| monetization § refunds                     | No automatic refund after the pools or the draw open; the zero-games refund is retired. Pre-draw removal, cancelled event and eviction keep their automatic paths.                                                                                                                                                                      |
